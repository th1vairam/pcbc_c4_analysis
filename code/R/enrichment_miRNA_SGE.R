...
thisFileName <- 'enrichment_miRNA_SGE.R'
...
DMAP = merge(DMAP, human_ensg2symbol, by = 'ensembl_gene_id', all=T)
############################################################################################################
...
ALL_USED_IDs = c(ALL_USED_IDs, DMIRNA.ALL_ID)
...
Comparisons.ALL = lapply(Comparisons.ALL, function(x, DMAP){
  return(
    unique(DMAP$hgnc_symbol[DMAP$hsa_id %in% x$GeneSymbol])
    )
  }, DMAP)
...
Comparisons.EACH = lapply(Comparisons.EACH, function(x, DMAP){return(unique(DMAP$hgnc_symbol[DMAP$hsa_id %in% x$GeneSymbol]))}, DMAP)
...
logCPM_ID = 'syn4595977'
ALL_USED_IDs = c(ALL_USED_IDs, logCPM_ID)
logCPM = data.table::fread(synGet(logCPM_ID)@filePath, data.table=F)
backGroundGenes = unique(DMAP$hgnc_symbol[DMAP$hsa_id %in% logCPM$GeneName])
############################################################################################################

############################################################################################################
#### Filter gene list ####
GeneSets = filterGeneSets(GeneSets, backGroundGenes, minSize = 10, maxSize = 10000)
############################################################################################################

############################################################################################################
#### Perform enrichment analysis ####
# Perform enrichment analysis
enrichResults.miRNA = list()
...
enrichResults.miRNA[[args]] = as.data.frame(rbindlist(tmp))
enrichResults.miRNA[[args]]$FDR = p.adjust(enrichResults.miRNA[[args]]$pval,'fdr')
enrichResults.miRNA[[args]]$adj.pval = p.adjust(enrichResults.miRNA[[args]]$pval,'bonferroni')
writeLines(paste0('Completed ',args))  


# Write results to file
for(name in names(enrichResults.miRNA))
  enrichResults.miRNA[[name]]$ComparisonName = name
enrichmentResults = as.data.frame(rbindlist(enrichResults.miRNA))
...
write.table(enrichmentResults, file = paste(gsub('[^[:alnum:]]','_',args),'enrichmentResults_miRNA.tsv',sep='.'), sep='\t', row.names=F)
collectGarbage()
############################################################################################################
...
algo = 'Fisher'
ENR_OBJ = File(paste(gsub('[^[:alnum:]]','_',args),'enrichmentResults_miRNA.tsv',sep='.'), name = paste(gsub('[^[:alnum:]]','_',args),'miRNA'), parentId = 'syn4907671')
