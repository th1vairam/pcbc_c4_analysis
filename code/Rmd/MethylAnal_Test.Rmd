...
# knitFile2Synapse(file = "./MethylAnal_Test.Rmd", name = 'Methylation Analysis Mixed Effects', owner = 'syn4231747', overwrite=F)
...
FactorCovariates = c("Diffname_short", "BeadChip", "Row", "Column", "PassageAtThaw", 
                     "PassageAtDNAHarvest", "Cell_Line_Type", "Cell_Line_of_Origin", "Tissue_of_Origin", 
                     "Reprogramming_Gene_Combination", "Culture_Conditions", "Small_Molecules", 
                     "Other_Conditions_During_Reprogramming", "Donor_Life_Stage", "Race", "Ethnicity", "Gender", 
                     "Disease", "Originating_Lab", "Donor_ID", "Cell_Type_of_Origin_Level2",
...
```
### Probe filtering
* Remove probes with low variance (least 25%)
* Remove probes with beta between 0.4 and 0.6 in all the samples
```{r gene.filtering, cache=TRUE, fig.width=8, fig.height=8, echo=TRUE}
# Remove bottom 25% varying probes
exp.variance = apply(as.matrix(EXP),1,var)
hist(log10(exp.variance), main = 'Variance distribution')
ind.remove <- (exp.variance <= sort(exp.variance)[round(0.25*length(exp.variance))])

# Remove probes with beta between 0.4 and 0.6 in all the samples
ind.remove <- ind.remove | (rowSums(EXP >= 0.4 & EXP <= 0.6)/dim(EXP)[2] == 1)
...
colnames(DESIGN$design) = sapply(colnames(DESIGN$design),function(x){substr(x,1,30)})
...
```{r iterative.adjusted.voom.normalisation, include=FALSE}
...
#                           repositoryPath=paste0('code/Rmd/', thisFileName))
  ...
  # Code
  CODE <- File('./MethylAnal_Test.Rmd',name = 'Methylation Analysis Mixed Effects',parentId = parentId)
