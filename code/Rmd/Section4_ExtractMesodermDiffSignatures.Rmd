---
title: Markers/signatures of mesodermal differentiaiton
...
knit2synapse::knitToFolderEntity(file = 'Section4_ExtractMesodermDiffSignatures.Rmd',
                                 parentId = "syn7162983",
                                 entityName = "4 Signatures of Mesodermal Differentiation")
```
...
thisFileName = 'Section4_ExtractMesodermDiffSignatures.Rmd'
...
### Indentify mesodermal differentiation markers (time course)
Here we define the markers as genes, miRNAs and methylation probes that are differentially expressed between a given differentiation transition (i.e., SC-MESO5, MESO5-MESO15, MESO15-MESO30) 
```{r markers}
...
diffexp.all = diffexp %>%
  ...
  unique %>%
  dplyr::mutate(from.to.state = paste(from.state, to.state, sep = '_')) %>%
  dplyr::filter(from.to.state %in% c('SC_MESO5', 'MESO5_MESO15', 'MESO15_MESO30'))

# Extract specific signatures
specific.signatures = diffexp.all %>%
  group_by(feature, feature.assay, feature.direction) %>%
  summarise(ncount.from.to = length(unique(from.to.state)), from.to.state = paste(unique(from.to.state), collapse = ',')) %>%
  filter(ncount.from.to == 1) %>%
  data.frame

writeLines('Number of entities that are differentially expressed between any one transition cluster with respect to all the other are')
kable(specific.signatures %>%
        group_by(from.to.state, feature.assay, feature.direction) %>%
        summarise(ncount = length(unique(feature))) %>%
        spread(from.to.state, ncount))
```
...
mrna.cov = covariates[['mrna']] %>%
  filter(Diffname_short %in% c('SC','MESO5', 'MESO15', 'MESO30'))
rownames(mrna.cov) = mrna.cov$UID

clust = specific.signatures %>%
  filter(feature.assay == 'mrna', feature.direction == 1) %>%
  dplyr::rename(hgnc_symbol = feature, cluster = from.to.state) %>%
...
methyl.cov = covariates[['methyl']] %>%
  filter((Diffname_short %in% c('SC','MESO5')))
...
methyl.cov = do.call(rbind, methyl.cov[c('SC','MESO5')])
colnames(methyl.cov) = gsub('Diffname_short','DifferentiationStage', colnames(methyl.cov))
methyl = do.call(cbind, methyl[c('SC','MESO5')])
...
# Side Annotation
ha1 = HeatmapAnnotation(clust[,'cluster',drop=F], which = 'row',
                        col = list('cluster' = c('SC_MESO5' = 'violet')))
...
CODE <- Folder(name = '4 Signatures of Mesodermal Differentiation', parentId = 'syn7162983')
CODE <- synStore(CODE)
...
obj = File('Signatures.tsv', 
           name = 'Mesodermal Specific Signatures',
