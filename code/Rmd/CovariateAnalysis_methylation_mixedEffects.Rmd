...
# knitFile2Synapse(file = "./CovariateAnalysis_methylation_mixedEffects.Rmd", name = 'Covariate Analysis Methylation Mixed Effects All', owner = 'syn4486559', overwrite=F)
...
* Remove probes with beta less than 0.4 in all the samples or beta greater than 0.6 in all the samples
...
loopCount = 0 
while(length(residualSigCovars$significantCovars)!=0 && loopCount <= 0){
...
EXPR <- rownameToFirstColumn(EXPR,'methProbeID')  
write.table(EXPR,file = './Filtered_methylation_Expression.tsv',sep='\t',row.names=F,col.names=T,quote=F)
...
RESID.EXPR <- rownameToFirstColumn(RESID.EXPR,'methProbeID')  
write.table(RESID.EXPR,file = './Adjusted_methylation_Expression.tsv',sep='\t',row.names=F,col.names=T,quote=F)
RESID.EXPR_OBJ <- File('./Adjusted_methylation_Expression.tsv',name = 'Adjusted Expression',parentId = CODE$properties$id)
RESID.EXPR_OBJ <- synStore(RESID.EXPR_OBJ, used = ALL_USED_IDs, activityName = ActivityName, executed = thisFile)  
...
|  *Results*           |  *SynapseID*                                                                         |
|  -------             |   ---------                                                                          |
|  Adjusted Expression |  `r paste(RESID.EXPR_OBJ$properties$id,EXPR$properties$versionNumber,sep='.')`       |
