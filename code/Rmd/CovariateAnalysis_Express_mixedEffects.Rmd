...
if (SYNAPSE_STORE){  
  ...
  COUNTS <- File('./Processed_CPM_Counts.tsv',name = 'Processed Counts Express Mixed Effects',parentId = parentId)
  COUNTS <- synStore(COUNTS, used = ALL_USED_IDs, activityName = ActivityName, executed = CODE$properties$id)  
  
  # Adjusted Expression Data
  RESID.EXPR <- rownameToFirstColumn(RESID.EXPR,'GeneName')  
  write.table(RESID.EXPR,file = './Adjusted_Expression.tsv',sep='\t',row.names=F,col.names=T,quote=F)
  RESID.EXPR <- File('./Adjusted_Expression.tsv',name = 'Adjusted Expression Express Mixed Effects',parentId = parentId)
  RESID.EXPR <- synStore(RESID.EXPR, used = ALL_USED_IDs, activityName = ActivityName, executed = CODE$properties$id)  
...
|  *Results*           |  *SynapseID*                  |
|  -------             |   ---------                   |
|  Adjusted Expression |  `r RESID.EXPR$properties$id` |