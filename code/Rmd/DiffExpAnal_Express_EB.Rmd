...
topGenes <- topTable(fit2, coef="Cell_Line_TypeESC-Cell_Line_TypeiPSC", 
                     adjust.method = "BH", p=p.thresh, n=1e6)
```
...
PC <- prcomp(VOOM.ADJUSTED.GENE_EXPRESSION$E[rownames(topGenes), ], 
...
* Design only with Cell Line Type and technical covariates (lane), and
* Design with Cell Line Type, Donor ID and technical covariates (lane)
...
for (design in c('withoutDonorID','withDonorID')){
  if(design == 'withoutDonorID'){    
    DESIGN <- getDesignMatrix(COVARIATES[,c('Cell_Line_Type','lane')], Intercept = F)
  } else {
    DESIGN <- getDesignMatrix(COVARIATES[,c('Cell_Line_Type','Donor_ID','lane')], Intercept = F)
  }
  ...
  # Adjust model with new design
  VOOM.ADJUSTED.GENE_EXPRESSION = voom(PROCESSED_COUNTS$filteredExprMatrix, design = DESIGN$design, plot=F) 
  
  # Fitting linear model
  FIT <- lmFit(VOOM.ADJUSTED.GENE_EXPRESSION)
               
               $E, design = DESIGN$design, weights = VOOM.ADJUSTED.GENE_EXPRESSION$weights)
  DIFF.EXP[[design]]$FIT <- FIT
  ...
  DIFF.EXP[[design]]$FIT.CONTRAST <- FIT.CONTRAST
...
hist(PVAL[,2],main="adj.P.val distribution for ESC-iPSC withoutDonor", xlab="PVAL")
hist(PVAL[,3],main="adj.P.val distribution for ESC-iPSC withDonor", xlab="PVAL")

tmp <- merge(filter(PVAL[,c(1,3)],PVAL[,3]<=0.05),
             filter(FC[,c(1,3)],PVAL[,3]<=0.05),
             by='GeneNames')
colnames(tmp) <- c('GeneNames','ESC-iPSC.withDonor.pval','ESC-iPSC.withDonor.FC')
kable(tmp)
```
Eight genes are significantly differentially expressed between iPSc and hESc

Principal components with selected 8 genes
```{r pca}
# Find principal components of expression to plot
tmp <- filter(PVAL,PVAL[,3]<=0.05)
PC <- prcomp(scale(VOOM.ADJUSTED.GENE_EXPRESSION$E[tmp$GeneNames,]))
...
if (SYNAPSE_STORE){  
  ...
  CODE <- synStore(CODE, used = ALL_USED_IDs, activityName = ActivityName, executed = 'https://github.com/th1vairam/pcbc_c4_analysis/blob/797bcdb641a4918deeb98b11cfa947aad74c7021/code/Rmd/DiffExpAnal_Express_EB.Rmd')
