...
all.int$feature.assay[is.na(all.int$feature.assay)] = 'NONE'syn7346460
...
weighted.signatures$MESO = dplyr::filter(diffexp, from.state == 'SC', to.state %in% c('MESO15', 'MESO30'), 
                                         feature %in% signatures[['MESO']]) %>%
  dplyr::select(feature, to.state, adj.P.value, logFC) %>%
  ddply(.(feature, to.state), .fun = function(y){-log10(y$adj.P.value)*abs(y$logFC)}) %>%
  daply(.(feature), .fun = function(y){max(y$V1, na.rm=T)})
...
intersect(key.regulators.unw$DE$global.regulators,
          key.regulators.w$DE$global.regulators)

setdiff(key.regulators.w$DE$global.regulators, 
        key.regulators.unw$DE$global.regulators)
