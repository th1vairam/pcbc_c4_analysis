---
title: "Covariate Analysis of iPSC lines"
...
# knit2synapse(file = "./CovariateAnalysisIpsc.Rmd", owner = 'syn3576534', wikiName = "Covariate Analysis iPSC",overwrite=F)
...
# Remove all non-iPSC lines
ind = unique(c(ind,which(METADATA$Cell_Line_Type == "ESC"))) 
...
FactorCovariates <- setdiff(FactorCovariates,names(which(apply(METADATA[,FactorCovariates],2,function(x){length(levels(factor(x)))}) == 1)))
...
p <- p + theme_bw() + scale_size_manual(values = c(4, 2)) + theme(legend.position="top") 
p
```
...
if (SYNAPSE_STORE){  
  ActivityName <- 'Covariate Analysis of iPSC lines'
  
  # Code
  CODE <- File('./CovariateAnalysisIpsc.Rmd',name = 'Covariate Analysis iPSC',parentId = parentId)
  ...
  # Expression Data
  write.table(VOOM.ADJUSTED.GENE_EXPRESSION$E,file = './AdjustedExpressionIpsc.tsv',sep='\t',row.names=T,col.names=T,quote=F)
  EXPRESSION <- File('./AdjustedExpressionIpsc.tsv',name = 'Adjusted Expression iPSC',parentId = parentId)
  EXPRESSION <- synStore(EXPRESSION, used = ALL_USED_IDs, activityName=ActivityName, executed=CODE$properties$id)  
  
  # Adjustment Weights 
  write.table(VOOM.ADJUSTED.GENE_EXPRESSION$weights,file = './AdjustedWeightsIpsc.tsv',sep='\t',row.names=T,col.names=T,quote=F)
  WEIGHTS <- File('AdjustedWeightsIpsc.tsv',name = 'Adjusted Weights iPSC',parentId = parentId)
  WEIGHTS <- synStore(WEIGHTS, used = ALL_USED_IDs, activityName=ActivityName, executed=CODE$properties$id)
  
  # Design Matrix
  write.table(VOOM.ADJUSTED.GENE_EXPRESSION$design,file = './AdjustedDesignIpsc.tsv',sep='\t',row.names=T,col.names=T,quote=F)
  DESIGN <- File('AdjustedDesignIpsc.tsv',name = 'Adjusted Design iPSC',parentId = parentId)
