...
# knitFile2Synapse(file = "./CovariateAnalysis_methylation_mixedEffects_lessStringent.Rmd", name = 'Covariate Analysis Methylation Mixed Effects LessStringent', owner = 'syn4380469', overwrite=F)
...
loopCount = 0 
while(length(residualSigCovars$significantCovars)!=0 && loopCount <= 1){
...
```{r residualSigCovars, fig.width=22, fig.height=14}
residualSigCovars[["PC_res"]][[1]]$plotData
```
```{r residualSigCovars.ALL, fig.width=24, fig.height=14}
residualSigCovars[["PC_res"]][[2]]$plotData
```
### Calculate Residuals
```{r residualisation, fig.height=8, fig.width=8}
...
thisFileName <- 'CovariateAnalysis_methylation_mixedEffects_lessStringent.Rmd'
...
# Code
CODE <- File('./CovariateAnalysis_methylation_mixedEffects_lessStringent.Rmd',name = 'Covariate Analysis Methylation Mixed Effects LessStringent',parentId = parentId)
CODE <- synStore(CODE, used = ALL_USED_IDs, activityName = ActivityName, executed = thisFile)

# Adjusted Expression Data
EXP <- rownameToFirstColumn(RESID.EXP,'GeneName')  
write.table(EXP,file = './Adjusted_MethylationExpression_lessStringent.tsv',sep='\t',row.names=F,col.names=T,quote=F)
RESID.EXPR <- File('./Adjusted_MethylationExpression_lessStringent.tsv',name = 'Adjusted Expression Methylation Mixed Effects LessStringent',parentId = parentId)
...
write.table(COVARIATES,file = './Covariates_Methylation_MixedEffects_lessStringent.tsv',sep='\t',row.names=F,col.names=T,quote=F)
COVARIATES <- File('Covariates_Methylation_MixedEffects_lessStringent.tsv',name = 'Covariates Methylation Mixed Effects LessStringent',parentId = parentId)
...
write.table(DESIGN,file = './AdjustedDesign_Methylation_MixedEffects_lessStringent.tsv',sep='\t',row.names=F,col.names=T,quote=F)
DESIGN <- File('AdjustedDesign_Methylation_MixedEffects_lessStringent.tsv',name = 'Adjusted Design Methylation Mixed Effects LessStringent',parentId = parentId)
