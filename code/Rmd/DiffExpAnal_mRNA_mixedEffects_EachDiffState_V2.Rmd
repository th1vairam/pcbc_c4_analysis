---
title: "Differential Network Analysis for eXpress aligned mRNA data with mixed effect modeling performed at indivudal differentiation stages"
...
getDifferentialExpression <- function(DiffState, DATA){
  ...
  writeLines('Number of differentially expressed genes at adj.P.Val <= 0.05 and logFC >= 1 or <= -1')
...
gctset = rbindlist(lapply(DEXP, function(x){ DEXP$DE$DIFF.EXP}))
ALL_USED_IDs = unlist(lapply(DATA, function(x){return(x$ALL_USED_IDs)}))
```
