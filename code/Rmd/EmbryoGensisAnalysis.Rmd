...
p = lapply(orderedDiffState, function(diffState, emb){
  ...
  tmp = tmp %>% tidyr::gather(Control, N.genes, -Comparison) %>%
    ...
    plyr::rename(c('Comparison' = 'DiffState'))
  tmp = rbind(tmp, data.frame(DiffState = diffState, N.genes=0, Control = 'TF'))
  tmp$Control[tmp$Control=='NA'] = 'None'
