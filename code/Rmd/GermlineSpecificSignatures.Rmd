---
title: Identifying molecular signatures of differentiation states from inter omic assays (i.e., mRNA, miRNA, DNA methylation probes and splicing junctions)
...
covariates.table.id = c(mrna = 'syn3156503', mirna = 'syn3219876', methyl = 'syn3156828', splicing = 'syn3156503')
...
```{r plot.pca, fig.height=10, fig.width=10}
plotdata = plotdata %>%
  left_join(covariates.table %>% 
...
pl = list()
for (assays in c('mrna','mirna','methyl','splicing')){
...
```
```{r plot.pca1, fig.height=5, fig.width=10}
writeLines('Plots for SC and EB alone')
for (assays in c('mrna','mirna','methyl','splicing')){
  pl = list()
  for (diffstate in c('EB', 'SC')){
    p = ggplot(filter(plotdata, assay == assays, DifferentiationStage == diffstate), 
               aes(x = PC1, y = PC2, color = DifferentiationStage, shape = DifferentiationStage)) 
    p = p + geom_point(stroke = 0.75) + theme_bw() + theme(legend.position = 'top')
    p = p + geom_text(aes(label=ifelse(DifferentiationStage == 'EB' | DifferentiationStage == 'SC', C4_Cell_Line_ID, ''), hjust=0, vjust=0))
    p = p + ggtitle(assays) + scale_size_manual(values = 4) 
    pl[[diffstate]] = p
  }
  multiplot(plotlist = pl, cols = 2)
...
for (assays in c('mrna','mirna','methyl','splicing')){
  # PC1
  PC1 = filter(plotdata, assay == assays) %>% 
    dplyr::select(UID, C4_Cell_Line_ID, DifferentiationStage, PC1) %>% 
    spread(DifferentiationStage, PC1) %>%
    ddply(.(C4_Cell_Line_ID), .fun = function(x){
      apply(x[,-(1:2)], 2, mean, na.rm = T)
    })
  rownames(PC1) = PC1$C4_Cell_Line_ID

  # PC2
  PC2 = filter(plotdata, assay == assays) %>% 
    dplyr::select(UID, C4_Cell_Line_ID, DifferentiationStage, PC2) %>% 
    spread(DifferentiationStage, PC2) %>%
    ddply(.(C4_Cell_Line_ID), .fun = function(x){
      apply(x[,-(1:2)], 2, mean, na.rm = T)
    })
  rownames(PC2) = PC2$C4_Cell_Line_ID

  writeLines(paste('Correlation between SC and EB ordering in',assays))
  print(cor(PC1[,'SC'], PC2[rownames(PC1), 'EB'], use = 'pairwise.complete.obs') %>% as.numeric())
  
  all.pc1[[assays]] = PC1; all.pc2[[assays]] = PC2
}

# PC1 mrna
PC1.mrna = filter(plotdata, assay == 'mrna') %>% 
  dplyr::select(UID, C4_Cell_Line_ID, DifferentiationStage, PC1) %>% 
  spread(DifferentiationStage, PC1) %>%
  ddply(.(C4_Cell_Line_ID), .fun = function(x){
    apply(x[,-(1:2)], 2, mean, na.rm = T)
  })
rownames(PC1.mrna) = PC1.mrna$C4_Cell_Line_ID

# PC2 mrna
PC2 = filter(plotdata, assay == 'mrna') %>% 
  dplyr::select(UID, C4_Cell_Line_ID, DifferentiationStage, PC2) %>% 
  spread(DifferentiationStage, PC2) %>%
  ddply(.(C4_Cell_Line_ID), .fun = function(x){
    apply(x[,-(1:2)], 2, mean, na.rm = T)
...
writeLines('Correlation between EBs in mrna and methylation')
tmp1 = all.pc2[['mrna']][,'EB']
names(tmp1) = all.pc2[['mrna']][,'C4_Cell_Line_ID']
tmp2 = all.pc1[['methyl']][,'EB']
names(tmp2) = all.pc1[['methyl']][,'C4_Cell_Line_ID']

cor(tmp1, tmp2[names(tmp1)], use = 'pairwise.complete.obs') %>% as.numeric()

writeLines('Correlation between EBs in mrna and mirna')
tmp1 = all.pc2[['mrna']][,'EB']
names(tmp1) = all.pc2[['mrna']][,'C4_Cell_Line_ID']
tmp2 = all.pc2[['mirna']][,'EB']
names(tmp2) = all.pc2[['mirna']][,'C4_Cell_Line_ID']

cor(tmp1, tmp2[names(tmp1)], use = 'pairwise.complete.obs') %>% as.numeric()

writeLines('Correlation between SCs in mrna and methylation')
tmp1 = all.pc1[['mrna']][,'SC']
names(tmp1) = all.pc1[['mrna']][,'C4_Cell_Line_ID']
tmp2 = all.pc1[['methyl']][,'SC']
names(tmp2) = all.pc1[['methyl']][,'C4_Cell_Line_ID']

cor(tmp1, tmp2[names(tmp1)], use = 'pairwise.complete.obs') %>% as.numeric()

writeLines('Correlation between SCs in mrna and mirna')
tmp1 = all.pc1[['mrna']][,'SC']
names(tmp1) = all.pc1[['mrna']][,'C4_Cell_Line_ID']
tmp2 = all.pc2[['mirna']][,'SC']
names(tmp2) = all.pc2[['mirna']][,'C4_Cell_Line_ID']
