...
changeBeta = apply(compNames.new, 1, function(x, covariates, betaFraction){  
  ind1 = which(covariates[,x['class']] == x["variable1Short"])
  ind2 = which(covariates[,x['class']] == x["variable2Short"])
  
  if (length(ind1) > 0 && length(ind2) > 0 ){
    var1Mean = rowMeans(betaFraction[,covariates$UID[ind1], drop=F], na.rm=T)
    var2Mean = rowMeans(betaFraction[,covariates$UID[ind2], drop=F], na.rm=T)
...
file.obj = File("changeInBeta.RData", name = "Change in Beta", parentId = parentId)
