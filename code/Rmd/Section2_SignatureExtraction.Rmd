...
expr.mean = mapply(function(cov, exp){
  cov %>% dlply(.(Diffname_short), .fun = function(x, exp){
    data.frame(feature = exp[,1],
               DF = rowMeans(exp[,x$UID], na.rm = T)) %>%
      plyr::rename(c('DF' = unique(x$Diffname_short)))
  }, exp) %>%
    join_all(type = 'full') %>%
    tidyr::gather(state, mn.expr, -feature)
}, covariates, expr, SIMPLIFY = F) %>%
...
diffexp$mrna = downloadFile(diffexp.id['mrna']) %>%
  ...
  left_join(expr.mean %>%
              dplyr::rename(from.state = state, from.mn.expr = mn.expr)) %>%
  left_join(expr.mean %>%
              dplyr::rename(to.state = state, to.mn.expr = mn.expr)) %>%
  ddply(.(feature, from.state, to.state), .fun = function(x){
    x$mx.mn.expr = max(x$from.mn.expr, x$to.mn.expr)
    return(x)
  }) %>%
  dplyr::filter(adj.P.value <= pval.th, abs(logFC) >= mrna.lfc.th, mx.mn.expr > mrna.lcpm) %>%
  dplyr::select(feature, from.state, from.mn.expr, to.state,  to.mn.expr, Assay, adj.P.value, logFC) 
...
diffexp$mirna = downloadFile(diffexp.id['mirna']) %>%
  ...
  dplyr::filter(adj.P.value <= pval.th, abs(logFC) >= mirna.lfc.th, mx.mn.expr > mirna.lcpm) %>%
  dplyr::select(feature, from.state, from.mn.expr, to.state,  to.mn.expr, Assay, adj.P.value, logFC) 
...
diffexp$methyl = downloadFile(diffexp.id['methyl']) %>%
  ...
  left_join(expr.mean %>%
              dplyr::rename(to.state = state, to.mn.expr = mn.expr)) %>%
  dplyr::filter(adj.P.value <= pval.th, abs(changeBETA) >= methyl.changeBeta.th) %>%
  ddply(.(feature, from.state, to.state), .fun = function(x){
    x$mx.mn.expr = max(x$from.mn.expr, x$to.mn.expr)
    return(x)
  }) %>%
  dplyr::filter(mx.mn.expr > methyl.beta) %>%
  dplyr::select(feature, from.state, from.mn.expr, to.state, to.mn.expr, Assay, adj.P.value, logFC, changeBETA) 

diffexp = rbindlist(diffexp, use.names = T, fill = T)

diffexp = rbindlist(list(diffexp,
                         diffexp %>%
                           dplyr::rename(to.state = from.state, from.state = to.state,
                                         to.mn.expr = from.mn.expr, from.mn.expr = to.mn.expr) %>%
                           dplyr::mutate(
                             logFC = -logFC, changeBETA = -changeBETA)),
                    use.names = T, fill =T) %>%
...
tmp = specific.signatures %>%
  group_by(from.state, feature.assay, feature.direction) %>%
  summarise(ncount = length(unique(feature)))

tmp1 = specific.signatures %>%
  group_by(from.state, feature.assay, feature.direction) %>%
...
geneSetsToTest = geneSetsToTest[sapply(geneSetsToTest, length) != 0]
...
# Print top results to wiki
enrichResults %>% 
  # tidyr::separate(MarkerSetName, c('from.state','feature.direction','feature.assay'), sep = '\\.') %>%
  filter(pval <= 0.01, noverlap >= 5) %>% 
  dplyr::select(MarkerSetName, GeneSetName, OR) %>%
  dplyr::mutate(OR = as.numeric(OR)) %>%
  group_by(MarkerSetName) %>%
  top_n(10, OR) %>%
  spread(MarkerSetName, OR) %>%
  kable
```
