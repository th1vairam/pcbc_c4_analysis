...
rownames(EXP) = EXP[,1]
EXP = EXP[sample(dim(EXP)[1], 10000),-(1)]
...
# Get number of significantly differentialy expressed genes with adj.P.Val <= 0.05 and logFC <= -0.4
...
Number of differentially expressed genes at FDR <= 0.05 and logFC >= 0.4 or logFC <= -0.4
...
GCT_OBJ <- File('./DiffExpAnal_methylation_mixedEffects_limma_diffexp.tsv', name = 'Differentially Expressed Genes', parentId = CODE$properties$id)
...
COMP_OBJ <- File('./DiffExpAnal_methylation_mixedEffects_limma_comparisons.tsv', name = 'Differentially Expressed Genes comparison list', parentId = CODE$properties$id)
...
| Differentially Expressed Genes    |  `r paste(GCT_OBJ$properties$id,GCT_OBJ$properties$versionNumber,sep='.')` |
