...
# knitFile2SynapseFolder(name = 'Differential Analysis Methylation Mixed Effects LessStringent', owner = 'syn4380469', overwrite=F)
...
write.table(,file='./DiffnameShort_intercept_methylation_mixedEffects.tsv',sep='\t',row.names=F,quote=F)
...
write.table(,file='./DiffnameShort_pval_methylation_mixedEffects.tsv',sep='\t',row.names=F,quote=F)
...
# Store results: fval
FVAL = lapply(DIFF.EXP, function(x){return(rownameToFirstColumn(x$DEXPP[,'fval', drop=F], 'methProbeID'))})
FVAL = join_all(FVAL, by = 'methProbeID', match = 'all')
colnames(PVAL)[-(1)] = names(DIFF.EXP)
FVAL = merge(FVAL, EXP[,c('methProbeID','symbol','entrezID')], all = T, by = 'methProbeID')
  
write.table(,file='./DiffnameShort_fval_methylation_mixedEffects.tsv',sep='\t',row.names=F,quote=F)
FVAL_OBJ <- File('./DiffnameShort_fval_methylation_mixedEffects.tsv',name = 'Differential Expression Analysis Methylation Mixed Effects fval',parentId = CODE$properties$id)
FVAL_OBJ <- synStore(FVAL_OBJ, used = ALL_USED_IDs, activityName = activityName, executed=thisFile)
```
|  *Results*                           |  *SynapseID*                |
|  ----------------------------------- |   ------------------------  |
|  Differential Expression (intercept) |  `r INT_OBJ$properties$id`  |
|  Differential Expression (pvalue)    |  `r PVAL_OBJ$properties$id` |
|  Differential Expression (fvalue)    |  `r FVAL_OBJ$properties$id` |