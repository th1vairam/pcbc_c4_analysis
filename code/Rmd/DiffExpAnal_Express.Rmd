...
# knit2synapse(file = "./DiffExpAnal_Express.Rmd", owner = '', wikiName = "Differential Expression Analysis Express",overwrite=T)
...
#### Residuals
Regress out run, land and index (all technical covariates) for differential expression analysis
```{r residual.calculation, echo=FALSE, include=FALSE}
varsToRemove <- colnames(DESIGN)[c(grep('run',colnames(DESIGN)),grep('lane',colnames(DESIGN)),grep('index',colnames(DESIGN)))]
varsToAddBackIn <- setdiff(colnames(DESIGN),varsToRemove)

RESIDUAL.EXPRESSION = calcResiduals(as.matrix(EXPR), as.matrix(DESIGN), sampleWeights = as.matrix(WEIGHTS), varsToAddBackIn = varsToAddBackIn)

NEW.DESIGN <- DESIGN[,varsToAddBackIn]
```
...
DIFFNAME <- tapply(Gender,Gender,diffExpAnal,as.matrix(EXPR),as.matrix(NEW.DESIGN),as.matrix(WEIGHTS),COVARIATES)
```
...
kable(NUM.SIG.SETS)
```
No difference in differentiation between male and female stem cells
...
if(SYNAPSE_STORE){
  ...
  FC <- rownameToFirstColumn(DIFFNAME$all$logFC,'GeneNames')
  ...
  PVAL <- rownameToFirstColumn(DIFFNAME$all$adj.P.Val,'GeneNames')
  write.table(PVAL,file='./DiffnameShort_Pvalue_Express.tsv',sep='\t',row.names=F,quote=F)
  FC <- File('./DiffnameShort_Pvalue_Express.tsv',name = 'Differential Expression Analysis Express pvalue',parentId = parentId)
  FC <- synStore(FC, used = ALL_USED_IDs, activityName = activityName, executed=CODE$properties$id)
}
```