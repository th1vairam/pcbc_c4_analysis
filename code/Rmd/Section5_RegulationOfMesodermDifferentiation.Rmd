---
title: Regulatory Network of Mesoderm Differentiation 
...
knit2synapse::knitToFolderEntity(file = 'Section5_RegulationOfMesodermDifferentiation.Rmd',
                                 parentId = "syn7162983",
                                 entityName = "5 Regulatory Networks of Mesoderm Differentiation")
```
...
thisFileName = 'Section5_RegulationOfMesodermDifferentiation.Rmd'
...
CODE <- Folder(name = '5 Regulatory Networks of Mesoderm Differentiation', parentId = 'syn7162983')
CODE <- synStore(CODE)
```
...
```{r get.markers}
state.specific.signatures = downloadFile('syn7450284')
ALL_USED_IDs = c(ALL_USED_IDs, 'syn7450284')
```
...
coexpression = downloadFile(coexp.id) %>%
  ...
  dplyr::mutate(from.to.state = paste(from.state, to.state, sep = '_')) %>%
  dplyr::filter(from.to.state %in% c('MESO5_SC', 'MESO15_MESO5', 'MESO30_MESO15'))
...
singleton.int = diffexp %>%
  dplyr::mutate(from.to.state = paste(from.state, to.state, sep = '_')) %>%
  dplyr::filter(from.to.state %in% c('SC_MESO5', 'MESO5_MESO15', 'MESO15_MESO30')) %>% 
  dplyr::filter(Assay != 'splicing') %>%
  ddply(.(from.to.state), .fun = function(x, allInt){
...
obj = synStore(obj, 
               activityName = 'Mesoderm Network Identification', 
               executed = thisFile, 
...
all.graphs = all.int %>%
  dlply(.(from.to.state), .fun = function(x){
    g <- x %>%
...
all.graphs[c(1:2)] = NULL

# Extract signatures
signatures = dlply(state.specific.signatures, .(from.to.state), .fun = function(x){unique(x$feature)})
...
}, all.graphs, signatures[names(all.graphs)], SIMPLIFY = F)
names(key.regulators.unw) = names(all.graphs)
...
weighted.signatures = lapply(names(signatures), function(x, Signatures, DiffExp){
  DiffExp %>%
    dplyr::mutate(from.to.state = paste(from.state, to.state, sep = '_')) %>%
    dplyr::filter(from.to.state == x, feature %in% Signatures[[x]]) %>%
    dplyr::select(feature, adj.P.value, logFC) %>%
    daply(.(feature), .fun = function(y){-log10(y$adj.P.value)*abs(y$logFC)})
}, signatures, diffexp)
...
}, all.graphs, weighted.signatures[names(all.graphs)], SIMPLIFY = F)
names(key.regulators.w) = names(all.graphs)
