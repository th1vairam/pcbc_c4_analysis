...
# Post adjusted design matrix
DESIGN = getDesignMatrix(COVARIATES[,c("Cell_Type_of_Origin_Level2", "Gender", "Diffname_short","Donor_Life_Stage","lane","Culture_Conditions","run")], Intercept = F)
DESIGN = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
...
# Post adjusted design matrix
DESIGN = getDesignMatrix(COVARIATES[,c("Culture_Conditions", "Cell_Type_of_Origin_Level2", "Gender", "Diffname_short","Donor_Life_Stage","lane","run")], Intercept = F)
DESIGN = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
...
# Obtain all the differential expession combinations
CULT.COND <- list()
CULT.COND$logFC <- data.frame(row.names = rownames(EXPR$E))
CULT.COND$adj.P.Val <- data.frame(row.names = rownames(EXPR$E))
...
CULT.COND$SIG.EXP.POS <- CULT.COND$adj.P.Val<=0.05 & CULT.COND$logFC >= 1   
CULT.COND$NUM.SIG.EXP.POS <- colSums(CULT.COND$SIG.EXP.POS)

# Get number of significantly differentialy expressed genes with adj.P.Val <= 0.05 and logFC <= -1
CULT.COND$SIG.EXP.NEG <- CULT.COND$adj.P.Val<=0.05 & CULT.COND$logFC <= -1   
...
if(SYNAPSE_STORE){
  ...
  write.table(DEXPP.GENES,file='./DiffnameShort_DiffGenes_Express_mixedEffects.gct',sep='\t',row.names=F,quote=F)
  DEXPP <- File('./DiffnameShort_DiffGenes_Express_mixedEffects.gct',name = 'Differential Expression Analysis Express Mixed Effects GeneList',parentId = parentId)
  ...
  SIG.SETS_OBJ <- File('./AllSignificantGenes_Express_mixedEffects.tsv',name = 'Differential Expression Analysis Express Mixed Effects Comparison',parentId = parentId)
