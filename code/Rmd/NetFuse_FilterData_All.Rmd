...
# knitToFolderEntity(file = "./NetFuse_FilterData_All.Rmd", parentId = 'syn4821700', entityName = 'Filtered Data')
...
### Merge metadata
```{r merge.metadata}
METADATA = lapply(METADATA, function(METADATA, COM.COLS, NUID){
  METADATA = unique(METADATA[,COM.COLS])
  rownames(METADATA) = METADATA$NUID
  METADATA = METADATA[NUID,]
  METADATA = METADATA[,c('NUID',setdiff(colnames(METADATA),'NUID'))]
  }, COM.COLS)
kable(sapply(METADATA,dim))
```
...
for (name in names(COUNTS)){
  ...
  COUNT <- synStore(COUNT, used = as.character(ALL_USED_IDs), activityName = ActivityName, executed = thisFile)  
}
