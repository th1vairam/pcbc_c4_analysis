...
metadata_filtered <- 
  ...
  filter(Originating_Lab %in% c('Gadue, French, & Weiss','Not Applicable','Torok-Storb','Zambidis')) %>%
...
p <- p + geom_point(aes(color=Cell_Type_of_Origin, shape=Gender, size=Cell_Line_Type))
p <- p + theme_bw() + scale_size_manual(values = c(5, 3)) + theme(legend.position="top") 
p
```
