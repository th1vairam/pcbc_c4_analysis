...
doDiffExpr <- function(PROC_COUNT, DESIGN, WEIGHTS, COVARIATES, comparison) {
  # Calculate correlation between donors
  correlation <- duplicateCorrelation(PROC_COUNT, design = DESIGN,
                                      weights = WEIGHTS, block=COVARIATES$Donor_ID)
  
  # Fit linear model using mixed effects design
  FIT = lmFit(PROC_COUNT, design = DESIGN, weights = WEIGHTS, 
              block=COVARIATES$Donor_ID, correlation = correlation2$cor)
  
  CONT.NAMES2 <- colnames(DESIGN)[grep(comparison, colnames(DESIGN2))]
  ...
  foo <- list()
  foo$logFC <- data.frame(row.names = rownames(PROC_COUNT))
  foo$adj.P.Val <- data.frame(row.names = rownames(PROC_COUNT))
  foo$SIG.SETS <- data.frame()
  
  for (i in colnames(CONT)){
    tmp <- topTable(FIT.CONTRAST, coef=i, number=dim(PROC_COUNT)[1])    
...
#### Fixed Effects
Diffname\_short, Gender, Cell\_Line\_Type, Reprogramming\_Gene\_Combination, Cell\_Type\_of\_Origin\_Level2, Culture\_Conditions, run, Donor\_Life\_Stage
#### Random Effects
Donor\_ID
#### Differentiation stages
```{r diffstate, cache=TRUE, include=FALSE}
# Post adjusted design matrix
DESIGN = getDesignMatrix(COVARIATES[,c("Diffname_short","Gender","Cell_Line_Type",
                                       "Reprogramming_Gene_Combination",
                                       "Cell_Type_of_Origin_Level2",
                                       "Culture_Conditions","run", "Donor_Life_Stage")], 
                         Intercept = F)
...
# Calculate correlation between donors
correlation <- duplicateCorrelation(PROC_COUNT, design = DESIGN, weights = WEIGHTS, block=COVARIATES$Donor_ID)

# Fit linear model using mixed effects design
FIT = lmFit(PROC_COUNT, design = DESIGN, weights = WEIGHTS, block=COVARIATES$Donor_ID, correlation = correlation$cor)
...
# Obtain all the differential expession combinations
DIFFNAME <- list()
DIFFNAME$logFC <- data.frame(row.names = rownames(PROC_COUNT))
DIFFNAME$adj.P.Val <- data.frame(row.names = rownames(PROC_COUNT))
...
# Obtain all the differential expession combinations
GENDER <- list()
GENDER$logFC <- data.frame(row.names = rownames(PROC_COUNT))
GENDER$adj.P.Val <- data.frame(row.names = rownames(PROC_COUNT))
...
# Obtain all the differential expession combinations
REP.GENE.COMBO <- list()
REP.GENE.COMBO$logFC <- data.frame(row.names = rownames(PROC_COUNT))
REP.GENE.COMBO$adj.P.Val <- data.frame(row.names = rownames(PROC_COUNT))
...
CELL.TYPE.OF.ORIGIN$logFC <- data.frame(row.names = rownames(PROC_COUNT))
CELL.TYPE.OF.ORIGIN$adj.P.Val <- data.frame(row.names = rownames(PROC_COUNT))
...
# Obtain all the differential expession combinations
CULT.COND <- list()
CULT.COND$logFC <- data.frame(row.names = rownames(PROC_COUNT))
CULT.COND$adj.P.Val <- data.frame(row.names = rownames(PROC_COUNT))
