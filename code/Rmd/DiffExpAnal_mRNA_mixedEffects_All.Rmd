...
correlation <- duplicateCorrelation(EXPR, design = DESIGN, weights = WEIGHTS, block=COVARIATES$Donor_ID)
...
DONOR.LIFE.STAGE$logFC <- data.frame(row.names = rownames(EXPR$E))
DONOR.LIFE.STAGE$adj.P.Val <- data.frame(row.names = rownames(EXPR$E))
