...
doDiffExpr <- function(PROC_COUNT, DESIGN, WEIGHTS, block, comparison) {
  ...
  CONT.NAMES2 <- colnames(DESIGN)[grep(comparison, colnames(DESIGN))]
...
newVars <- rotateVars(postAdjustedCovars, comparison)
newCovariates <- COVARIATES[, newVars]

DESIGN = getDesignMatrix(newCovariates, Intercept = F)
DESIGN = DESIGN$design[, linColumnFinder(DESIGN$design)$indepCols]

DIFFNAME <- doDiffExpr(PROC_COUNT = PROC_COUNT, DESIGN = DESIGN,
...
```{r gender, cache=TRUE}
# Post adjusted design matrix
DESIGN = getDesignMatrix(COVARIATES[,c("Gender","Diffname_short",
                                       "Reprogramming_Gene_Combination",
                                       "Cell_Type_of_Origin_Level2",
...
CONT.NAMES2 <- colnames(DESIGN)[grep('Gender', colnames(DESIGN2))]
CONT.NAMES <- combn(CONT.NAMES, 2)
CONT.NAMES <- apply(CONT.NAMES, 2, paste, collapse='-')
...
# Post adjusted design matrix
DESIGN = getDesignMatrix(COVARIATES[,c("Reprogramming_Gene_Combination",
                                       "Gender", "Diffname_short",
                                       "Cell_Type_of_Origin_Level2",
                                       "Culture_Conditions", "run",
                                       "Donor_Life_Stage")],
                         Intercept = F)
