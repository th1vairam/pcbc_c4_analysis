...
```
### Enrichment Analysis
Perform enrichment analysis for differentiation stage.
...
writeInputFiles <- function(cont.name, FC, PVAL, inputFolderName){
  keepRows <- FC[,cont.name]>=1 & PVAL[,cont.name]<=0.05
  input <- FC[keepRows, c('GeneNames', cont.name), drop=F]
  ...
  write.table(input, paste(inputFolderName,FName,sep='/'),
  ...
  Genes <- rbind(Genes,
                 cbind(paste(str_replace_all(cont.name,'[^[:alnum:]]','_'),'down',sep='_'),
                       paste(input$GeneNames,collapse=',')))
  
  FName <- paste(str_replace_all(cont.name,'[^[:alnum:]]','_'),'down','txt',sep='.')  
  
  write.table(input,paste(inputFolderName,FName,sep='/'), sep='\t',
