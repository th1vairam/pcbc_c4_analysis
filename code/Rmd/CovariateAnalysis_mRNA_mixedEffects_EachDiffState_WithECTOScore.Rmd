---
title: "Covariate Analysis for eXpress aligned mRNA data with mixed effect modeling performed at indivudal differentiation stages (with ECTO-MESO scores of EB)"
...
# knit2synapse::knitToFolderEntity(file = "./CovariateAnalysis_mRNA_mixedEffects_EachDiffState_WithECTOScore.Rmd",
#                                  entityName = 'Covariate Analysis mRNA Mixed Effects EachDiffState WithECTOScore',
...
METADATA = inner_join(METADATA, cibersort.scores) %>%
  dplyr::select(C4_Cell_Line_ID, ecto.scores) %>%
  group_by(C4_Cell_Line_ID) %>%
  summarise(ecto.scores = mean(ecto.scores, na.rm=T)) %>%
  right_join(METADATA)
METADATA$ecto.scores[is.na(METADATA$ecto.scores)] = 0.5
```
### Filter data
...
thisFileName <- 'CovariateAnalysis_mRNA_mixedEffects_EachDiffState_WithECTOScore.Rmd'
...
CODE <- Folder(name = 'Covariate Analysis mRNA Mixed Effects EachDiffState WithECTOScore', 
               parentId = parentId)
CODE <- synStore(CODE)
