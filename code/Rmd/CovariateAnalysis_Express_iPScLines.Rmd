---
title: "Covariate Analysis for eXpress aligned data (only for iPSc lines)"
...
# knit2synapse(file = "./CovariateAnalysis_Express_iPScLines.Rmd", owner = 'syn3631186', wikiName = "Covariate Analysis Express iPScLines",overwrite=T)
...
FactorCovariates = c('Diffname_short','run','lane','index','Cell_Type_of_Origin', 'Cell_Line_of_Origin', 'Tissue_of_Origin', 'Reprogramming_Vector_Type','Reprogramming_Gene_Combination', 'Donor_Life_Stage','Originating_Lab','Gender','Donor_ID','Disease','Race','Ethnicity')
ContCovariates = NULL
```
...
if (SYNAPSE_STORE){  
  ActivityName <- 'Covariate Analysis of eXpress aligned data for iPSc lines'
  
  # Code
  CODE <- File('./CovariateAnalysis_Express_iPScLines.Rmd',name = 'Covariate Analysis Express iPScLines',parentId = parentId)
  CODE <- synStore(CODE, used = ALL_USED_IDs, activityName = ActivityName, executed = 'https://github.com/th1vairam/pcbc_c4_analysis/blob/d62425e7d7535094c083e6665ad387b35d6f4203/code/Rmd/CovariateAnalysis_Express_iPScLines.Rmd')
  ...
  write.table(EXP,file = './AdjustedExpression_Express_iPScLines.tsv',sep='\t',row.names=F,col.names=T,quote=F)
  EXPRESSION <- File('./AdjustedExpression_Express_iPScLines.tsv',name = 'Adjusted Expression Express iPScLines',parentId = parentId)
  ...
  write.table(WEIGHTS,file = './AdjustedWeights_Express_iPScLines.tsv',sep='\t',row.names=F,col.names=T,quote=F)
  WEIGHTS <- File('AdjustedWeights_Express_iPScLines.tsv',name = 'Adjusted Weights Express iPScLines',parentId = parentId)
  ...
  write.table(COVARIATES,file = './Covariates_Express_iPScLines.tsv',sep='\t',row.names=F,col.names=T,quote=F)
  COVARIATES <- File('Covariates_Express_iPScLines.tsv',name = 'Covariates Express iPScLines',parentId = parentId)
  ...
  write.table(DESIGN,file = './AdjustedDesign_Express_iPScLines.tsv',sep='\t',row.names=F,col.names=T,quote=F)
  DESIGN <- File('AdjustedDesign_Express_iPScLines.tsv',name = 'Adjusted Design Express iPScLines',parentId = parentId)
