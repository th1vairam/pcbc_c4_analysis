...
# FileName = 'OUTLIER ANALYSIS PLOTS.pdf'
# pdf(FileName,width=40,height=20)
...
```{r hclustmds, fig.width=20, fig.height=20, tidy=FALSE}
...
potentialOutliers <- unique(c('SC13-054.182.3.7','SC12-040.420.12.19','SC13-043DE.419.12.8','SC12-024ECTO.420.12.12', 'SC13-057iECScramble.219.6.22','SC12-034MESO.219.1.7', 'SC12-039MESO.419.12.27', 'H9.144.7.7',METADATA$UID[intersect(which(!METADATA$pass_qc),grep('133',METADATA$UID))]))
...
p <- p + scale_size_manual(values = c(14, 7))
p <- p + scale_color_brewer(palette = "Set1")
p <- p + scale_shape_manual(values = c(15, 5, 16, 17, 1, 18))
p <- p + geom_text(aes(label=labels), size=7, hjust=0)
...
3. SC13−054.182.3.7, SC12−040.420.12.19, SC13-057iECScramble.219.6.22, H9.133.1.11, SC11-013A.133.5+6.10, SC11-009A.133.2.14, SC11-010.A.133.5+6.8, SC11-008A.133.2.13, SC11-012A.133.2.9 were observed to be outliers
...
1. MESO5 tend to cluster with DE samples than MESO15 and 30 which cluster with EB
2. Somatic samples cluster seperately
3. SC13−054.182.3.7, SC12−040.420.12.19, SC13-057iECScramble.219.6.22, H9.133.1.11, SC11-013A.133.5+6.10, SC11-009A.133.2.14, SC11-010A.133.5+6.8, SC11-008A.133.2.13, SC11-012A.133.2.9 were observed to be outliers
...
```{r storefiles}
Outliers <- c('SC13-054.182.3.7','SC12-040.420.12.19','SC13-057iECScramble.219.6.22','H9.133.1.11','SC11-013A.133.5+6.10','SC11-009A.133.2.14','SC11-010A.133.5+6.8','SC11-008A.133.2.13','SC11-012A.133.2.9')
write.table(Outliers,file = './Outliers_mRNA.txt',sep='\t',col.names=F,row.names=F,quote=F)

if (SYNAPSE_STORE){
  OUTLIER_CODE <- File(path = './OutlierAnalysis.Rmd',parentId = parentId, synapseStore = T, used = c(COUNT_OBJ,METADATA_OBJ),activityName='OutlierAnalysis',
       executed='')
  knit2synapse(file = "./OutlierAnalysis.Rmd", owner = OUTLIER_CODE$properties$id, wikiName = "Outlier Analysis")
  OUTLIERS <- File(path = './Outliers_mRNA.txt',parentId = parentId, synapseStore = T, used = c(COUNT_OBJ,METADATA_OBJ),activityName='OutlierAnalysis',
                   executed=OUTLIER_CODE)  
}
```
