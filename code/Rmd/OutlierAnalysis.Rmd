...
#plotdata$labels <- plotdata$UID
potentialOutliers <- c('SC13-054.182.3.7','SC12-040.420.12.19','SC13-043DE.419.12.8',
                       'SC12-024ECTO.420.12.12', 'SC13-057iECScramble.219.6.22',
                       'SC12-034MESO.219.1.7', 'SC12-039MESO.419.12.27', 'H9.144.7.7')

plotdata$labels <- ifelse(plotdata$UID %in% potentialOutliers, as.character(plotdata$UID),"")

# plotdata$labels <- ifelse(grepl("MESO", plotdata$Diffname_short), as.character(plotdata$UID),"")

# plotdata$labels <- ifelse(plotdata$UID %in% c("SC13-054.182.3.7", "SC12-040.420.12.19"),
#                           as.character(plotdata$UID),
#                           "")

p <- ggplot(plotdata, aes(x=C1, y=C2)) + geom_point(aes(color=factor(cluster), 
                                                        size=pass_qc, shape=plotshape))
#p <- scale_color_continuous(low="red", high="green")
...
1. MESO-5 tend to cluster with DE samples than MESO-15 and 30
2. Somatic samples cluster differently
3. SC13−054.182.3.7, SC12−040.420.12.19 were observed to be outliers
4. SC12-040 has abnormal karyotype (but not marked exclude and passes qc)
5. SC13-054 did not pass QC
6. SC13-043DE.419.12.8 is a mislabel
