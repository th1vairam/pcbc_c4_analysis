---
title: "Covariate Analysis for eXpress aligned data with mixed effect modeling for iPSc lines"
...
# source('/mnt/Github/Knit2Synapse/knitFile2Synapse.R')
# knitFile2Synapse(file = "./CovariateAnalysis_Express_mixedEffects_iPScLines.Rmd", name = "Covariate Analysis Express Mixed Effects iPSC", owner = '', overwrite=F)
...
* Remove samples from hESc cell lines
...
# Drop univarying covariates
toRetain <- names(COVARIATES)[sapply(COVARIATES,function(x){length(levels(x))}) != 1]
COVARIATES <- COVARIATES[,toRetain]
```
...
while(length(residualSigCovars$significantCovars)!=0){
  ...
  residualSigCovars = runPCAandPlotCorrelations(RESIDUAL.GENE_EXPRESSION, 
                                                select(DESIGN,-starts_with('Donor_ID')),
...
Reperform normalisation with custom design matrix (manually)
...
if (SYNAPSE_STORE){  
  ActivityName <- 'Covariate Analysis of eXpress aligned data with mixed effects modeling for iPSC lines'
  
  # Code
  CODE <- File('./CovariateAnalysis_Express_mixedEffects_iPScLines.Rmd', name = 'Covariate Analysis Express Mixed Effects iPSC',parentId = parentId)
  ...
  write.table(EXP,file = './AdjustedExpression_Express_mixedEffects_iPScLines.tsv',sep='\t',row.names=F,col.names=T,quote=F)
  EXPRESSION <- File('./AdjustedExpression_Express_mixedEffects_iPScLines.tsv',name = 'Adjusted Expression Express Mixed Effects iPSC',parentId = parentId)
  ...
  write.table(WEIGHTS,file = './AdjustedWeights_Express_mixedEffects_iPScLines.tsv',sep='\t',row.names=F,col.names=T,quote=F)
  WEIGHTS <- File('AdjustedWeights_Express_mixedEffects_iPScLines.tsv',name = 'Adjusted Weights Express Mixed Effects iPSC',parentId = parentId)
  ...
  write.table(COVARIATES,file = './Covariates_Express_mixedEffects_iPScLines.tsv',sep='\t',row.names=F,col.names=T,quote=F)
  COVARIATES <- File('Covariates_Express_mixedEffects_iPScLines.tsv',name = 'Covariates Express Mixed Effects iPSC',parentId = parentId)
  ...
  write.table(DESIGN,file = './AdjustedDesign_Express_mixedEffects_iPScLines.tsv',sep='\t',row.names=F,col.names=T,quote=F)
  DESIGN <- File('AdjustedDesign_Express_mixedEffects_iPScLines.tsv',name = 'Adjusted Design Express Mixed Effects iPSC',parentId = parentId)
