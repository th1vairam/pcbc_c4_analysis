---
title: "Covariate Analysis for eXpress aligned mRNA data with mixed effect modeling performed at indivudal differentiation stages using globally identified signatures of SC and Germ layers"
...
knit2synapse::knitToFolderEntity(file = 'CovariateAnalysis_mRNA_mixedEffects_EachDiffState_WithSignatures.Rmd',
                                 parentId = "syn5008933",
                                 entityName = 'Covariate Analysis mRNA Mixed Effects EachDiffState WithSignatures')
```
...
library(CovariateAnalysis) ## Get dev branch from github.com/th1vairam
library(data.table)
library(tidyr)
library(plyr)
library(dplyr)
...
Genomic signatures comes from the global analysis `r SIGNATURES_ID`
...
```{r get.signatures, cache=FALSE, include=TRUE}
# Get signatures
SIGNATURES_OBJ = synGet(SIGNATURES_ID)
ALL_USED_IDs <- c(ALL_USED_IDs, SIGNATURES_ID)
SIGNATURES = read.table(SIGNATURES_OBJ@filePath, sep = '\t', header=T, quote='') %>%
  filter(feature.assay %in% c('mrna','TF')) %>%
  dlply(.(from.state), .fun = function(x){
    as.character(unique(unlist(x$feature)))
  })
SIGNATURES$MESO15 = SIGNATURES$MESO
SIGNATURES$MESO30 = SIGNATURES$MESO
SIGNATURES$MESO5 = SIGNATURES$MESENDO
SIGNATURES$DE = SIGNATURES$MESENDO
SIGNATURES$EB = c(SIGNATURES$MESENDO, SIGNATURES$ECTO, SIGNATURES$MESO)
SIGNATURES = SIGNATURES[c('SC','DE','MESO5','EB','ECTO','MESO15','MESO30')]
```
### Filter data
...
```{r cpmnormalisation}
PROCESSED_COUNTS <- dlply(METADATA, .(Diffname_short), .fun = function(mtd, counts, signatures){
  tmp = getGeneFilteredGeneExprMatrix(counts[, mtd$UID],
                                      MIN_GENE_CPM=1,                                
                                      MIN_SAMPLE_PERCENT_WITH_MIN_GENE_CPM=0.5)
  ftr = intersect(rownames(tmp$filteredExprMatrix$counts), signatures[[unique(mtd$Diffname_short)]])
  tmp = tmp$filteredExprMatrix$counts[ftr,]
}, COUNT, SIGNATURES)
```
...
initialVoomNormalisation <- function(covariates, processed_counts){
  ...
  gene.expression = voom(processed_counts[[diffState]], design=NULL, plot=T)
...
iterativeNormalisation <- function(covariates, processedCounts, voomExpr,
  ...
  while(length(residualSigCovars$significantCovars)!=0 && loopCount <= 15){
    ...
    GENE.EXPRESSION = voom(processedCounts[[diffState]], 
                           design = DM1$design, 
                           plot=F)
...
CODE <- Folder(name = 'Covariate Analysis mRNA Mixed Effects EachDiffState WithSignatures', 
               parentId = parentId)
CODE <- synStore(CODE)

storeFilesInSynapse <- function(covariates, 
                                processedCounts, 
                                postAdjustedCovars, 
                                code) {
  ...
  filteredCounts <- processedCounts[[diffState]]
  ...
  # # Adjustment Weights
  # weights <- cbind(rownames(processedCounts[[diffState]]),
  #                  postAdjustedCovars[[diffState]]$GENE.EXPRESSION$weights)
  # colnames(weights) <- c('GeneNames',
  #                        colnames(processedCounts[[diffState]]))
  # write.table(weights,
...
STORED.FILES = sapply(COVARIATES, 
                      storeFilesInSynapse, 
                      PROCESSED_COUNTS, 
                      VOOM.GENE.EXPRESSION.POSTADJUSTED, 
                      CODE)
```
### Results
```{r print.results}
