...
Following are the unique set of molecular features extracted from the 6 different comparisons between Sc and one of the 6 differentiation stages
```{r diffexp.summary}
writeLines('Number of entities that are differentially expressed between SC and anyother differentiation state are')
diffexp %>%
  filter(from.state == 'SC') %>%
  group_by(to.state, Assay, Direction) %>%
  summarise(count = length(unique(feature))) %>%
  dplyr::mutate(Direction = factor(Direction, labels = c('-1' = 'Down', '1' = 'Up'))) %>%
  tidyr::unite(Assay.Direction, Assay, Direction) %>%
  tidyr::spread(Assay.Direction, count) %>%
  kable
...
diffexp.int = inner_join(all.int,
                         diffexp %>%
                           ...
                           dplyr::rename(feature.assay = Assay, feature.fdr = adj.P.value, 
                                         feature.lfc = logFC, feature.cb = changeBETA, 
                                         feature.direction = Direction)) %>%
  inner_join(diffexp %>%
               dplyr::select(-from.mn.expr,-to.mn.expr) %>%
               dplyr::rename(target = feature, target.assay = Assay, 
                             target.fdr = adj.P.value, target.lfc = logFC, 
                             target.cb = changeBETA, target.direction = Direction)) %>%
  filter(feature.assay %in% c('methyl','mirna'), feature.direction != target.direction)
...
3. DE cluster together with MESODERM at day 5, whereas MESODERMS at day 15 and 30 cluster together and are more close to EB's than DE and SC
...
Inorder to test, whether our differentiation stages are unique and well chracterised, we checked the expression patterns of differentiation state specific genes that are present in the GO terms for DE, MESO, ECTO and SC 
...
p
```
As seen from the above heatmap, signatures of SC and ECTODERM shows higher expression in comparison to other differentiation stages. Also, MESODERMS at days 15 and 30 show either high or low expression for signature genes when comparing with other differentiation stages. However, MESODERM at day 5 and DEFINITIVE ENDODERM are clustered together and shows expression of both mesodermal and endodermal signatures. It is mainly due to the differentiation protocol used in our current analysis, which will generate cardiomyocyte specific mesoderms. Therefore for the remainder of the global analysis we are going to consider mesoderms at day 15 and 30 as mesoderms and DE as endoderm
