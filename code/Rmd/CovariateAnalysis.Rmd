...
# knit2synapse(file = "./CovariateAnalysis.Rmd", owner = 'syn3382512', wikiName = "Covariate Analysis",overwrite=T)
...
if (SYNAPSE_STORE){
  ...
  CODE <- synStore(CODE, used = c(COUNT_OBJ$properties$id,METADATA_OBJ@schema),activityName='Covariate Analysis', executed='https://github.com/th1vairam/pcbc_c4_analysis/blob/cov_anal/code/Rmd/CovariateAnalysis.Rmd')
  ...
  EXPRESSION <- synStore(EXPRESSION, used = c(COUNT_OBJ$properties$id,METADATA_OBJ@schema,CODE$properties$id), activityName='Covariate Analysis', executed='https://github.com/th1vairam/pcbc_c4_analysis/blob/cov_anal/code/Rmd/CovariateAnalysis.Rmd')
  ...
  WEIGHTS <- synStore(WEIGHTS, used = c(COUNT_OBJ$properties$id,METADATA_OBJ@schema,CODE$properties$id), activityName='Covariate Analysis', executed='https://github.com/th1vairam/pcbc_c4_analysis/blob/cov_anal/code/Rmd/CovariateAnalysis.Rmd')
  
  write.table(VOOM.ADJUSTED.GENE_EXPRESSION$design,file = './AdjustedDesign.tsv',sep='\t',row.names=T,col.names=T,quote=F)
  WEIGHTS <- File('AdjustedDesign.tsv',name = 'Adjusted Design',parentId = parentId)
  WEIGHTS <- synStore(WEIGHTS, used = c(COUNT_OBJ$properties$id,METADATA_OBJ@schema,CODE$properties$id), activityName='Covariate Analysis', executed='https://github.com/th1vairam/pcbc_c4_analysis/blob/cov_anal/code/Rmd/CovariateAnalysis.Rmd')  
}
```