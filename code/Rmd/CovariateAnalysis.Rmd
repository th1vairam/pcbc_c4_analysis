...
```{r synapsestore}
if (SYNAPSE_STORE){
  COVARIATES_CODE <- File('./CovariateAnalysis.Rmd',name = 'Covariate Analysis',parentId = parentId)
  COVARIATES_CODE <- synStore(COVARIATES_CODE, used = c(COUNT_OBJ$properties$id,METADATA_OBJ@schema),activityName='Covariate Analysis', executed='https://github.com/th1vairam/pcbc_c4_analysis/blob/cov_anal/code/Rmd/CovariateAnalysis.Rmd')
  DE <- File('./DifferentialExpression_DiffnameShort.txt',name = 'Differential Expression DiffnameShort',parentId = parentId)
  DE <- synStore(DE, used = c(COUNT_OBJ$properties$id,METADATA_OBJ@schema),activityName='Differential Analysis', executed=COVARIATES_CODE)
}
```
