...
getDifferentialExpression <- function(COVARIATES,COUNTS){
  ...
  CELL.LINE.TYPE$samples <- colnames(VOOM.ADJUSTED.GENE_EXPRESSION$E)
...
Correlation between PCs of expression from differentialy expressed genes and covariates 
```{r significant.covariates.all, cache=TRUE, fig.height=10, fig.width=15}
tmp <- sapply(RESULTS,function(x){print(x$sigCovars.all$PC_res[[2]]$plotData)})
```

Correlation between PCs of expression from differentialy expressed genes and covariates for only iPSC lines 
```{r significant.covariates.iPSC, cache=TRUE, fig.height=10, fig.width=15}
tmp <- sapply(RESULTS,function(x){print(x$sigCovars.iPSC$PC_res[[2]]$plotData)})
```
