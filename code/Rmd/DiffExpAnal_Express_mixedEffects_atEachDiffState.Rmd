...
* Fixed Effects: Diffname short, Gender, Cell Type of Origin Level2 and Culture Conditions
...
SIG.SETS <- lapply(colnames(FC)[-(1)],writeInputFiles, FC, PVAL, inputFolderName)
SIG.SETS <- plyr::ldply(SIG.SETS) 
colnames(SIG.SETS) <- c('Contrast.Names','Gene.Symbols')
...
write.table(DEXPP.GENES,file='./DiffnameShort_DiffGenes_Express_mixedEffects_atEachDiffState_genelist.tsv',sep='\t',row.names=F,quote=F)
DEXPP <- File('./DiffnameShort_DiffGenes_Express_mixedEffects_atEachDiffState_genelist.tsv',name = 'Differential Expression Analysis Express Mixed Effects At Each DiffState Genelist',parentId = parentId)
annotations(DEXPP) <- list(Pvalue='0.05',Foldchage='0')
DEXPP <- synStore(DEXPP, used = ALL_USED_IDs, activityName = activityName, executed=CODE$properties$id)
...
|  Differentially expressed genes (for enrichment) |  `r DEXPP$properties$id`  |
