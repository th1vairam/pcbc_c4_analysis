...
# knitFile2Synapse(file = "./DiffExpAnal_Express_mixedEffects_atEachDiffState.Rmd", owner = 'syn4104894', wikiName = "Differential Expression Analysis Express Mixed Effects iPScVsHEsc",overwrite=F)
...
#### Modeling with Cell_Line_Type as fixed effect and Donor_ID as random
```{r cell.line.type.factorisation, cache=TRUE}
getDifferentialExpression <- function(COVARIATES,COUNTS){
  Name <- paste('Diffname_short modeled', unique(COVARIATES$Diffname_short))
  print(Name)
  ...
  CELL.LINE.TYPE$voomSig <- VOOM.ADJUSTED.GENE_EXPRESSION[CELL.LINE.TYPE$SIG.EXP, ]

  DESIGN <- getDesignMatrix(COVARIATES,Intercept = F)
  DESIGN <- DESIGN$design
  colnames(DESIGN) <- gsub('[[:punct:]]','_',colnames(DESIGN))
  colnames(DESIGN) <- gsub(' ','_',colnames(DESIGN))
  EXPR <- VOOM.ADJUSTED.GENE_EXPRESSION$E[CELL.LINE.TYPE$SIG.EXP,rownames(COVARIATES)]
  if (CELL.LINE.TYPE$NUM.SIG.EXP !=0)
    CELL.LINE.TYPE$sigCovars.all <- runPCAandPlotCorrelations(EXPR,COVARIATES,Name)
  else
    CELL.LINE.TYPE$sigCovars.all <- NULL
  
  EXPR <- VOOM.ADJUSTED.GENE_EXPRESSION$E[CELL.LINE.TYPE$SIG.EXP,rownames(COVARIATES[COVARIATES$Cell_Line_Type == 'iPSC',])]
  if (CELL.LINE.TYPE$NUM.SIG.EXP !=0)
    CELL.LINE.TYPE$sigCovars.iPSC <- runPCAandPlotCorrelations(EXPR,COVARIATES,Name)
  else
    CELL.LINE.TYPE$sigCovars.iPSC <- NULL
...
PCA Plot for DE iPSC lines using genes differentially expressed between DE hESC and iPSC:

```{r pca}
# Find principal components of expression to plot
foo <- RESULTS[['DE']]$voom[RESULTS[['DE']]$SIG.EXP, ]

ipscsamples <- METADATA %>%
  filter(UID %in% colnames(foo)) %>%
  filter(Cell_Line_Type == "iPSC")

PC <- prcomp(foo[, ipscsamples$UID])
...
p <- p + geom_point(aes(color=Gender, shape=Reprogramming_Vector_Type), size=5)
p <- p + scale_size_manual(values = c(4, 3))
...
Correlation between PCs of expression from differentially expressed genes and covariates for only DE iPSC lines
```{r}
print(RESULTS[['DE']]$sigCovars.iPSC$PC_res[[2]]$plotData)
```


Correlation between PCs of expression from differentialy expressed genes and covariates  for only DE lines
```{r significant.covariates.all, cache=TRUE, fig.height=10, fig.width=15}
print(RESULTS[['DE']]$sigCovars.all$PC_res[[2]]$plotData)
```
...
if (SYNAPSE_STORE){  
  ...
  # Code
  CODE <- File('./DiffExpAnal_Express_mixedEffects_atEachDiffState.Rmd',name = 'KD Differential Expression Analysis Express Mixed Effects iPScVshESc',parentId = parentId)
  CODE <- synStore(CODE, used = ALL_USED_IDs, activityName = ActivityName, executed = 'https://github.com/kdaily/pcbc_c4_analysis/blob/252e172da541c3778dc15c32696864640d6d6326/code/Rmd/DiffExpAnal_Express_mixedEffects_atEachDiffState.Rmd')  
}
```