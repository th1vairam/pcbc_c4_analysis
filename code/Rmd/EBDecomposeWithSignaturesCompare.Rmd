...
CIBERSORT results with Thanneer's mRNA signature
```{r cibersort.output1, fig.height=8, fig.width=8}
cibersort.results.thanneer = downloadFile('syn7213213') %>%
  plyr::rename(c('Input Sample' = 'Input_Sample')) %>%
  dplyr::select(Input_Sample, MESO, ECTO, DE, MESO5) %>%
  gather(DiffState, Value, -Input_Sample) %>%
  dplyr::rename(Fraction = Value, ReferenceState = DiffState)

p = ggplot(cibersort.results.thanneer, aes(x = Input_Sample, y = Fraction, fill = ReferenceState)) + geom_bar(stat = 'identity')
p = p + theme(axis.text.x = element_text(angle = 45, hjust = 1))
p
```

CIBERSORT results with Nathan's mRNA signature
```{r cibersort.output2, fig.height=8, fig.width=8}
cibersort.results.nathan = downloadFile('syn7213212') %>%
  plyr::rename(c('Input Sample' = 'Input_Sample')) %>%
  dplyr::select(Input_Sample, MESO, ECTO, DE, MESO5) %>%
  gather(DiffState, Value, -Input_Sample) %>%
  dplyr::rename(Fraction = Value, ReferenceState = DiffState)

p = ggplot(cibersort.results.nathan, aes(x = Input_Sample, y = Fraction, fill = ReferenceState)) + geom_bar(stat = 'identity')
p = p + theme(axis.text.x = element_text(angle = 45, hjust = 1))
p

tmp = full_join(cibersort.results.thanneer %>% dplyr::rename(Thanneer.Fraction = Fraction),
                cibersort.results.nathan %>% dplyr::rename(Nathan.Fraction = Fraction)) %>%
  filter(ReferenceState == 'ECTO')

p = ggplot(tmp, aes(x = Thanneer.Fraction, y = Nathan.Fraction)) + geom_point() 
p = p + geom_smooth(method = lm)
p = p + geom_text(aes(label = Input_Sample), size = 2, hjust = 0, vjust = 0)
p
```

Association test between different covariates and stratified diff state scores of EBs (based only on Thanneers signature)
```{r associate.with.covars, fig.height=10, fig.width=10}
covars = covariates$mrna %>%
  filter(Diffname_short == 'EB') %>%
  dplyr::select(UID, Cell_Line_Type, Cell_Line_of_Origin, Tissue_of_Origin, 
  ...
  unique() %>%
  right_join( cibersort.results.thanneer %>% dplyr::rename(UID = Input_Sample) ) %>%
  as.data.frame() %>%
...
cont_covars = c("ECTO", "MESO","DE","MESO5")
...
```
A slight gender effect is observed

Association test between different covariates and stratified diff state scores of EBs (based only on Nathans signature)
```{r associate.with.covars1, fig.height=10, fig.width=10}
covars = covariates$mrna %>%
  filter(Diffname_short == 'EB') %>%
  dplyr::select(UID, Cell_Line_Type, Cell_Line_of_Origin, Tissue_of_Origin, 
                Reprogramming_Gene_Combination, Culture_Conditions, Donor_Life_Stage,
                Gender, Cell_Type_of_Origin_Level2, Reprogramming_Vector_Type) %>%
  unique() %>%
  right_join( cibersort.results.nathan %>% dplyr::rename(UID = Input_Sample) ) %>%
  as.data.frame() %>%
  spread(ReferenceState, Fraction)

factor_covars = c("UID","Cell_Line_Type", "Cell_Line_of_Origin", "Tissue_of_Origin", "Reprogramming_Gene_Combination", 
                  "Culture_Conditions", "Donor_Life_Stage", "Gender", "Cell_Type_of_Origin_Level2", "Reprogramming_Vector_Type")
cont_covars = c("ECTO", "MESO","DE","MESO5")
...
covars1[,factor_covars] = lapply(covars[,factor_covars], as.factor)
covars1[,cont_covars] = lapply(covars[,cont_covars], as.numeric)
rownames(covars1) = covars$UID
covars1$UID = NULL

COVARIATES.CORRELATION = getAssociationStatistics(covars1, PVAL = 0.1)
ggheatmap.show(COVARIATES.CORRELATION$plot, col.width=0.5, row.width=0.1)
```
No significant association with any covariates is observed