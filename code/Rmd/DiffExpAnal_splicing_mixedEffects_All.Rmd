...
# Input Parameters
PSI_ID = 'syn4991632' # Unprocessed file from alt-analyse pipeline
...
```
Obtain raw PSI, design, and covariates matrix from Synapse.
...
rownames(PSI) = PSI.ANNOT$"Minor-Isoform"
...
Synapse IDs of used data:
  | **Name**            | **SynapseID** |
  | PSI                 | `r paste(PSI_OBJ$properties$id, PSI_OBJ$properties$versionNumber,sep='.')` |
...
DIFFNAME <- doDiffExpr(countMat = PSI, designMat = DESIGN,
...
CELL.LINE.TYPE <- doDiffExpr(countMat=PSI, designMat=DESIGN,
                             weights=NULL, block=COVARIATES$Donor_ID,
                             comparison=comparison)

# Rename columns
CELL.LINE.TYPE$SIG.SETS = CELL.LINE.TYPE$SIG.SETS %>% plyr::rename(c("GeneSymbol" = "Minor-Isoform")) %>% left_join(PSI.ANNOT)
...
GENDER <- doDiffExpr(countMat=PSI, designMat=DESIGN,
                     weights=NULL, block=COVARIATES$Donor_ID,
                     comparison=comparison)

# Rename columns
GENDER$SIG.SETS = GENDER$SIG.SETS %>% plyr::rename(c("GeneSymbol" = "Minor-Isoform")) %>% left_join(PSI.ANNOT)
```
...
CELL.TYPE.OF.ORIGIN <- doDiffExpr(countMat=PSI, designMat=DESIGN,
                                  weights=NULL, block=COVARIATES$Donor_ID,
                                  comparison=comparison)
...
DONOR.LIFE.STAGE <- doDiffExpr(countMat=PSI, designMat=DESIGN,
                               weights=NULL, block=COVARIATES$Donor_ID,
                               comparison=comparison)

# Rename columns
DONOR.LIFE.STAGE$SIG.SETS = DONOR.LIFE.STAGE$SIG.SETS %>% plyr::rename(c("GeneSymbol" = "Minor-Isoform")) %>% left_join(PSI.ANNOT)
```
Get differentially spliced junctions using limma package with following coefficients in the linear model:
...
REP.GENE.COMBO <- doDiffExpr(countMat=PSI, designMat=DESIGN,
                             weights=NULL, block=COVARIATES$Donor_ID,
                             comparison=comparison)
...
CULT.COND <- doDiffExpr(countMat=PSI, designMat=DESIGN,
                        weights=NULL, block=COVARIATES$Donor_ID,
                        comparison=comparison)

# Rename columns
CULT.COND$SIG.SETS = CULT.COND$SIG.SETS %>% plyr::rename(c("GeneSymbol" = "Minor-Isoform")) %>% left_join(PSI.ANNOT)
```
Get differentially spliced junctions using limma package with following co-efficients  in the linear model:
