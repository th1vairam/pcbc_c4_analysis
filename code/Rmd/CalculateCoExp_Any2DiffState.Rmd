...
ALL_USED_IDs = c(ALL_USED_IDs, splicing = splicing_id)

PSI = downloadFile(splicing_id)
rownames(PSI) = PSI$`Minor-Isoform`
PSI = PSI[,-c(1,227:236)]

counts = c(counts, list(splicing = data.matrix(PSI)))
```
#### Get metadata matrices
```{r download.metadata}
#### Get metadata matrices
metadata_id = c(mrna = 'syn3156503', mirna = 'syn3219876', 
                methyl = 'syn3156828', splicing = 'syn3156503')
...
### Get previously calculated coexpression from synapse
```{r get.coexp, cache=FALSE}
correlation.old = downloadFile('syn5662375') 

# Calculate the difference in correlation between current and previously calculated correlations
allInteractions = allInteractions %>% 
  dplyr::mutate(feature.target= paste(feature, target, sep='_')) %>%
  dplyr::filter(!(feature.target %in% (paste(correlation.old$feature, correlation.old$target, sep = '_') %>% unique))) %>%
...
```{r combine.samples, cache=FALSE}
# Calculate the coexpression for remaining interactions
correlation.new = ddply(allInteractions, 
...
correlation = rbindlist(list(correlation.old, correlation.new), use.names = TRUE)
```

```{r synapse.store}
# Store metadata in synapse
write.table(correlation, file = 'correlationDiffComparison.tsv', sep = '\t', quote=F, row.names=F)
obj = File('correlationDiffComparison.tsv', name = 'Correlation coefficients', parentId = CODE$properties$id)
