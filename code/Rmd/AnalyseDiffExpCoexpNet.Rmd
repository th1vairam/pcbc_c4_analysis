...
=======
```{r getTFs}
>>>>>>> 8686dde840d02d14ed260454f26e68ea34b440e8
...
<<<<<<< HEAD
    dplyr::mutate(count = round(100*count/totalCount,2)) %>%
=======
...
diffexp.summary = lapply(names(diffexp), function(x, diffExp, totalCounts){
  x1 = diffExp[[x]] %>%
    dplyr::filter((Assay == 'TF' & abs(logFC) >= 1.098612) |
                    (Assay == 'mrna' & abs(logFC) >= 1.098612) |
                    (Assay == 'mirna' & abs(logFC) >= 1.098612) |
...
writeLines('Percentage differentially expressed at an FDR 0.05 and FC of 3 for mrna, mirna and change in beta of 0.35 for methylation probes and change in PSI of 0.2 for splicing junctions')
kable(dplyr::select(diffexp.summary, 
                    one_of(setdiff(colnames(diffexp.summary), 'totalCount')), 
                    totalCount))
```
...
for (i in 1:4){
<<<<<<< HEAD
  relative.control = lapply(names(diffexp), function(comp, diffExp, coExp, cocoexp){
    DE.target = diffExp[[comp]] %>%
      dplyr::filter((Assay == 'TF' & abs(logFC) >= 1.098612) |
                      (Assay == 'mrna' & abs(logFC) >= 1.098612))
    DE.feature = diffExp[[comp]] %>%
      dplyr::filter((Assay == 'TF' & abs(logFC) >= 1.098612) |
                      (Assay == 'mirna' & abs(logFC) >= 1.098612) |
                      (Assay == 'methyl' & abs(changeBETA) >= 0.35) |
                      (Assay == 'splicing' & abs(changePSI) >= 0.2))
    CE = dplyr::select(coExp, feature, target, Assay, one_of(comp)) %>%
      ...
      dplyr::filter(abs(correlation) >= cocoexp & feature %in% DE.feature$feature & target %in% DE.target$feature) %>%
      dplyr::filter(feature != target) %>%
      group_by(Assay) %>% summarise(count = 100*length(unique(target))/length(unique(DE.target$feature))) %>%
      setnames('count',comp)
    return(CE)
    }, diffexp, coexpression, COCoexp[i]) %>% join_all(type = "full")
    
    p[[i]] = lapply(orderedDiffState, function(diffState, relativeControl){
      ...
      tmp[,diffState] = 0
      tmp = dplyr::select(tmp, Assay, one_of(orderedDiffState)) %>%
        tidyr::gather(DiffState, Percentage, -Assay) %>%
        dplyr::mutate(DiffState = factor(DiffState, levels = orderedDiffState),
                      Assay = factor(Assay, levels = c('TF','mirna','methyl','splicing')))
      tmp = ggplot(tmp, aes(x = DiffState, y = Percentage))
      tmp = tmp + geom_bar(aes(fill = Assay),  stat = 'identity') 
      tmp = tmp + ggtitle(paste(diffState, paste0('Coexp=',COCoexp[i]), sep = ',')) 
      tmp = tmp + theme(axis.text.x = element_text(angle=90))
    },relative.control)
    writeLines(paste('At a FDR 0.05, absolute coexpression >=',COCoexp[i]))
    multiplot(plotlist = p[[i]], cols = 4)
}
dev.off()
=======
  p[[i]] = list()
...
writeLines(paste('At a FDR 0.05, absolute coexpression >=',COCoexp[2]))
multiplot(plotlist = p[[2]], cols=4)
writeLines('All percentages are percent of mrna differentially expresed at a FDR 0.05 and absolute FC greater than or equal to 3')
```

Relative control with top 1000 differentially expressed genes
```{r relativeControl3, include=FALSE}
relative.control = lapply(names(diffexp), function(comp, diffExp, coExp, n = 5000){
  DE.target = diffExp[[comp]] %>%
    dplyr::filter(Assay == 'TF' | Assay == 'mrna')
  DE.target$Rank = rank(-log10(DE.target$adj.P.value)) + rank(abs(DE.target$logFC))
  DE.target = DE.target[order(Rank, decreasing = T),]
  DE.target = DE.target[1:n,]
  DE.feature = diffExp[[comp]] %>%
    dplyr::filter((Assay == 'TF' & abs(logFC) >= 1.098612) |
                    (Assay == 'mirna' & abs(logFC) >= 1.098612) |
                    (Assay == 'methyl' & abs(changeBETA) >= 0.35) |
  ...
  CE = dplyr::select(coExp, feature, target, Assay, one_of(comp)) %>%
    ...
    dplyr::filter(abs(correlation) >= cocoexp & feature %in% DE.feature$feature & target %in% DE.target$feature) %>%
    dplyr::filter(feature != target) %>%
    group_by(Assay) %>% summarise(count = length(unique(target))) %>%
    setnames('count',comp)
  return(CE)
  }, diffexp, coexpression) %>% join_all(type = "full")

p = lapply(orderedDiffState, function(diffState, relativeControl){
  tmp = dplyr::select(relativeControl, Assay, contains(diffState))
  colnames(tmp) = gsub(diffState, '', colnames(tmp)) %>% gsub('_vs_','',.)
  tmp$Assay = gsub('_mrna','',tmp$Assay)
  tmp[,diffState] = 0
  ...
  tmp = ggplot(tmp, aes(x = DiffState, y = Percentage))
  tmp = tmp + geom_bar(aes(fill = Assay),  stat = 'identity') 
  tmp = tmp + ggtitle(paste(diffState,paste0('top ',n,' genes'), sep = ',')) 
  tmp = tmp + theme(axis.text.x = element_text(angle=90)) + ylim(0, 1500)
  },relative.control)
...
# Store all differentially expressed nodes in synapse
diffexp.counts=  lapply(filtered.graph, function(x){
  x$node
}) %>% rbindlist
write.table(diffexp.counts, file = 'filteredMedianCounts.tsv', sep = '\t', row.names=F, quote=F)
obj = File('filteredMedianCounts.tsv', name = 'Filtered Median Counts', parentId = CODE$properties$id)
obj = synStore(obj, executed = ThisFile, used = as.character(ALL_USED_IDs), activityName = ActivityName)
=======
>>>>>>> 8686dde840d02d14ed260454f26e68ea34b440e8
```
...
#### Extract sub-graphs
```{r extractSubGraph, results='asis'}
# Convert edgelist to graphs
...
#### Rank nodes in sub-graph
```{r calculateDegree, eval=FALSE}
# Calculate total degree and betweeness
nodeDegree = lapply(names(sub.graph.object), function(comp, subGraphs, filteredSubGraphs){
  filteredSubGraphs[[comp]]$
  totalStrength = igraph::strength(subGraphs[[comp]])
  betweennessDegree = igraph::betweenness(subGraphs[[comp]])
  all.degree = data.frame(feature = names(totalStrength),
    ...
    left_join(dplyr::select(filteredSubGraphs[[comp]], feature, Assay))
  ...
  return(all.degree)
<<<<<<< HEAD
}, sub.graph.object, filtered.sub.graph)
=======
}, counts)
>>>>>>> 8686dde840d02d14ed260454f26e68ea34b440e8
```
