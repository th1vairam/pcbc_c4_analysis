...
for (diffState1 in orderedDiffState){
  ...
  for (diffState2 in orderedDiffState){
    if (diffState1 != diffState2){
      ...
      # Get TF and nonTF specific modules
      TF.nonTF.mods = CE %>% filter(target.assay %in% 'TF' | target.assay %in% 'mrna')  %>%
        ...
        group_by(feature.assay, target.assay, CE.S, S) %>%
        summarise(target = paste(sort(unique(target)), collapse = ',')) %>%
        ungroup %>%
        dplyr::mutate(target.assay = gsub('mrna','nonTF',target.assay),
                      feature.assay = paste0('All_', feature.assay),
                      feature.assay = gsub('All_NA', 'None', feature.assay))
      
      # Get intersection modules
      TF.nonTF.mods1 = CE %>% filter(target.assay %in% 'TF' | target.assay %in% 'mrna')  %>%
        dplyr::mutate(CE.S = coexpression/abs(coexpression),
                      S = target.lfc/abs(target.lfc)) %>%
        dplyr::select(target, feature.assay, target.assay, CE.S, S) %>%
        group_by(target, target.assay, CE.S, S) %>%
        summarise(feature.assay = paste(sort(unique(feature.assay)), collapse = '_')) %>%
        group_by(feature.assay, target.assay, CE.S, S) %>%
        summarise(target = paste(sort(unique(target)), collapse = ',')) %>%
        ungroup %>%
        dplyr::mutate(target.assay = gsub('mrna','nonTF',target.assay),
                      feature.assay = paste0('Only_', feature.assay))
      TF.nonTF.mods1$feature.assay[TF.nonTF.mods1$feature.assay == 'Only_'] = 'None'
      
      # Combine and store module results
      tmp = rbind(TF.nonTF.mods, TF.nonTF.mods1) %>%
        unique
      
      if (all(c(diffState1, diffState2) == sort(c(diffState1, diffState2)))){
        tmp$CE.S[tmp$CE.S == 1] = 'CONCORDANTLY'; tmp$CE.S[tmp$CE.S == -1] = 'DISCORDANTLY'
        tmp$S[tmp$S == 1] = 'UP'; tmp$S[tmp$S == -1] = 'DOWN'
      } else {
        tmp$CE.S[tmp$CE.S == -1] = 'CONCORDANTLY'; tmp$CE.S[tmp$CE.S == 1] = 'DISCORDANTLY'
        tmp$S[tmp$S == -1] = 'UP'; tmp$S[tmp$S == 1] = 'DOWN'
      }
      ...
      modules[[diffState1]][[diffState2]] = tmp %>%
        dplyr::mutate(SetName = paste0(feature.assay, '_regulating_', S, '_regulated_', target.assay, '_', CE.S)) %>%
        dplyr::rename(Genes = target)
    }
  }
}

# Write all gene sets to synapse
tmp = lapply(modules, rbindlist, idcol = 'ToState') %>% 
  rbindlist(idcol = 'FromState') %>%
  dplyr::select(SetName, FromState, ToState, target.assay, CE.S, S, Genes)
...
```{r relativeControl5, fig.height=15, fig.width=15}
allBars = list()
for (diffState1 in orderedDiffState2){
  allBars[[diffState1]] = list()
  for (diffState2 in orderedDiffState2){
...
```{r sharedControl, fig.width=10, fig.height=15}
