---
title: "Coexpression Network Analysis for eXpress aligned mRNA data with only technical covariates (signed)"
...
#                    entityName = 'Coexpression Network Analysis mRNA All Technical Covariates Model Signed', 
#                    parentId = 'syn3256504', 
#                    overwrite=F)
...
softPower = 20
collectGarbage()
save(list=c('powerTables'), file = 'CoexppNet_mRNA_technicalCovariates_signed.RData')
```
Beta is chosen to be `r softPower`
```{r wgcna.adj, fig.width=15, fig.height=10}
# Get adjacency matrix
Adj = bicor(EXPR, nThreads = 2)
Adj = Adj/abs(Adj) * abs(Adj^softPower); # abs(Adj^softPower);#
collectGarbage()

# Get topological overlap matrix
TOM = TOMdist(Adj, TOMType='signed', verbose=3)
...
save(list=c('powerTables', 'Adj', 'TOM','tree', 'unmergedNetModules.colors'), 
     file = 'CoexppNet_mRNA_technicalCovariates_signed.RData')
```
...
CODE <- Folder(name = 'Coexpression Network Analysis mRNA All Technical Covariates Model Signed',parentId = parentId)
CODE <- synStore(CODE)
      
# Store Results
COEXPP_OBJ <- File('./CoexppNet_mRNA_technicalCovariates_signed.RData', name = 'Coexpression Networks', parentId = CODE$properties$id)
