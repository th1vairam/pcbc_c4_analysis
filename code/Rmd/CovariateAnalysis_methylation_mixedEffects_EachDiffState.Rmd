...
splitData <- function(cols,EXP,METADATA){
  ...
  exp.variance <- apply(as.matrix(EXP[,cols]),1,var)
  ind.remove1 <- exp.variance < sort(exp.variance)[round(0.25*length(exp.variance))]
  ind.remove2 <- (rowSums(EXP[,cols] <= 0.25)/dim(EXP[,cols])[2] == 1) | (rowSums(EXP[,cols] >= 0.75)/dim(EXP[,cols])[2] == 1)
  
  EXP <- EXP[!ind.remove1 & !ind.remove2,cols]  
