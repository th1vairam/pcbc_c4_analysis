...
potentialOutliers <- unique(c('SC13-054.182.3.7','SC12-040.420.12.19','SC12-024ECTO.420.12.12','SC12-034MESO.219.1.7',METADATA$UID[METADATA$Diffname_short=='SC' & METADATA$pass_qc == F], METADATA$UID[METADATA$C4_Karyotype_Result=='abnormal' & !is.na(METADATA$C4_Karyotype_Result)]))
...
3. As with cufflinks data, SC13−054.182.3.7, SC12−040.420.12.19, H9.133.1.11, SC11-013A.133.5+6.10, SC11-010A.133.5+6.8, SC11-008A.133.2.13, SC11-012A.133.2.9 samples were observed to be outliers
...
NOTE:
1. "SC11-002A.133.1.2"  "SC11-002B.133.1.4"  "SC11-003A.133.1.5"  "SC11-003B.133.1.6"  "SC12-040.420.12.19" were marked as abnormal karyotypes
2. SC12-040 has abnormal karyotype (but not marked exclude and passes qc)
3. SC13-054 did not pass QC

```{r storefiles}
Outliers <- c('SC13−054.182.3.7','SC12−040.420.12.19','H9.133.1.11','SC11-013A.133.5+6.10','SC11-010A.133.5+6.8','SC11-008A.133.2.13','SC11-012A.133.2.9')
write.table(Outliers,file = './Outliers_mRNA_Express.txt',sep='\t',col.names=F,row.names=F,quote=F)

if (SYNAPSE_STORE){
  OUTLIER_CODE <- File('./OutlierAnalysis_Express.Rmd',name = 'Outlier Analysis Express data',parentId = parentId)
  ...
  OUTLIERS <- File('./Outliers_mRNA_Express.txt',name= 'Outliers Express mRNA',parentId = parentId)
  OUTLIERS <- synStore(OUTLIERS, used = c(COUNT_OBJ$properties$id,METADATA_OBJ@schema),activityName='Outlier Analysis for Express data', executed=OUTLIER_CODE)  
}
```