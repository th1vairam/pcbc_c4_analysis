...
```{r predict.eb.pol.scores, include = FALSE}
ALL.DEXP = list()
ALL.AUC = c()
ALL.CF = list()
for(ncount in 1:100){
...
```{r predict.all, fig.width=6, fig.height=6}
...
```{r temp, fig.height=20, fig.width=15}
...
```{r predictors, fig.height=10, fig.width=15}
...
# Store differential expression
write.table(DEXP, file = 'DEXP_eb.pol.scores.tsv', sep = '\t', row.names = F, quote = F)
DEXP_OBJ = File('DEXP_eb.pol.scores.tsv', name = 'Differential Expression of SC against EB polarisation scores', parentId = CODE$properties$id)
DEXP_OBJ = synStore(DEXP_OBJ, used = ALL_USED_IDs, executed = thisFile, activityName = 'Predict EB polarisation')

# Store model coefficients
save(list = c('fit.cv','coef','ALL.AUC','ALL.DEXP','ALL.CF'), file = 'LR_Model.RData')
MOD_OBJ = File('LR_Model.RData', name = 'Logistic Regression Model for predicting EB polarisation scores', parentId = CODE$properties$id)
MOD_OBJ = synStore(MOD_OBJ, used = ALL_USED_IDs, executed = thisFile, activityName = 'Predict EB polarisation')
```