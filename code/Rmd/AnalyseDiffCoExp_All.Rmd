...
uniqueFeatures = lapply(TF.nonTF.fisher, function(x, diffexp){
  TFs = (intersect(x$TF, diffexp[['TF']]$feature))
  nonTFs = (intersect(x$nonTF, diffexp[['mirna']]$feature))
  x = dplyr::filter(x, TF %in% TFs | nonTF %in% nonTFs) %>%
...
set.seed(12345); clust.mirna.TF = kmeans(tmp.cor, 6)#clust.mirna.TF$nc)
...
h.mirna.TF = Heatmap(tmp.coExp, name = 'Corr', 
        ...
        show_row_names = F, row_names_side = 'right', show_column_names = T, show_row_hclust = F,
        heatmap_legend_param = figure_size)
draw(h.mirna.TF, heatmap_legend_side = "left", column_title = 'Transcription Factors-miRNA',
     column_title_gp = gpar(fontsize = 22, fontface = "bold"))
WGCNA::collectGarbage()
...
Performing analysis with `r dim(mirna.nonTF.coexp)[1]` nonTF-miRNA interactions calculated from `r dim(mirna.mrna.metadata)[1]` samples
...
clust.mirna.nonTF$cluster = clust.mirna.nonTF$cluster %>%
  factor(levels = c(10, 2, 1, 9, 6, 7, 5, 8, 3, 4))
...
Performing analysis with `r dim(methyl.mirna.coexp)[1]` miRNA-methlation interactions calculated from `r dim(methyl.mirna.metadata)[1]` samples
...
# Store coexpression
changeCoexp <- list(TF_nonTF = TF.nonTF.fisher,
