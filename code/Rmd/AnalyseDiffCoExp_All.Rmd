...
all.comparisons = t(combn(unique(mrna.mirna.metadata$Diffname_short), 2))
...
TF.mirna.fisher = apply(all.comparisons, 1, changeCoExp, 
                        ...
                        var1 = 'mrna', var2 = 'mirna', FDR = 0.05, logFC = 3)
names(TF.mirna.fisher) = apply(all.comparisons, 1, paste, collapse = '_vs_')

writeLines("Number of differentially co-expressed features at an FDR <= 0.05 and absolute logFC >= 3 are")
tmp = sapply(TF.mirna.fisher, function(x){dim(x)[1]}, simplify = F) %>% 
  ldply(function(x){data.frame(value = x)}, .id = 'Comparison')
kable(tmp)

# Extract co-expression matrix at each differentiation stage
uniqueFeatures = lapply(TF.mirna.fisher, function(x){
  dplyr::select(x, feature) %>%
    unlist
}) 
writeLines(paste("Total number of differentially co-expressed features at an FDR <= 0.05 and absolute logFC >= 3 are",
                 length(uniqueFeatures %>% unlist %>% unique)))
...
```{r cluster.TF.miRNA, fig.height=15, fig.width=15, results='asis', echo=FALSE}
tmp = TF.mirna.coexp %>%
...
# clust.TF.miRNA = pamk(tmp.cor, 1:20, seed = 12345, usepam= F)
set.seed(12345); clust.TF.miRNA = kmeans(tmp.cor, 10)#clust.TF.miRNA$nc)
clust.TF.miRNA$cluster = clust.TF.miRNA$cluster %>%
  factor(levels = c(2, 3, 7, 4, 5, 10, 9, 1, 8, 6))
  
col = rev(brewer.pal(5, "RdPu"));
figure_size = list(title_gp = gpar(fontsize = 18), labels_gp = gpar(fontsize = 14))

h.TF.miRNA = Heatmap(tmp.cor, name = 'Scaled Corr', column_title = 'Transcription Factors-miRNA', 
...
draw(h.TF.miRNA, heatmap_legend_side = "left")
WGCNA::collectGarbage()
```

### Calculate differential co-expression analysis between TF-methylation features at each diffstate
Performing analysis with `r dim(TF.methyl.coexp)[1]` TF-methylation features calculated from `r dim(mrna.methyl.metadata)[1]` samples
```{r coexpp.fisher.TF.methyl, fig.height=15, fig.width=15, echo=FALSE, results='asis'}
# Get all possible diff state comparisons
all.comparisons = t(combn(unique(mrna.methyl.metadata$Diffname_short), 2))

# Perform Fisher analysis across all possible diff state comparisons
TF.methyl.fisher = apply(all.comparisons, 1, changeCoExp, 
                        mrna.methyl.metadata, TF.methyl.coexp, 
                        var1 = 'mrna', var2 = 'methyl', FDR = 0.05, logFC = 3)
names(TF.methyl.fisher) = apply(all.comparisons, 1, paste, collapse = '_vs_')

writeLines("Number of differentially co-expressed features at an FDR <= 0.05 and absolute logFC >= 3 are")
tmp = sapply(TF.methyl.fisher, function(x){dim(x)[1]}, simplify = F) %>% 
  ldply(function(x){data.frame(value = x)}, .id = 'Comparison')
kable(tmp)

# Extract co-expression matrix at each differentiation stage
uniqueFeatures = lapply(TF.methyl.fisher, function(x){
  dplyr::select(x, feature) %>%
    unlist
}) 
writeLines(paste("Total number of differentially co-expressed features at an FDR <= 0.05 and absolute logFC >= 3 are",
...
```{r cluster.TF.methyl, fig.height=15, fig.width=15, results='asis', echo=FALSE}
tmp = TF.methyl.coexp %>%
  dplyr::filter(feature %in% (uniqueFeatures %>% unlist %>% unique)) %>%
...
writeLines(paste('Clustering',dim(tmp.cor)[1],'TF-methylation interactions'))
clust.TF.methyl = pamk(tmp.cor, 1:20, seed = 12345, usepam= F)
set.seed(12345); clust.TF.methyl = kmeans(tmp.cor, clust.TF.methyl$nc)
clust.TF.methyl$cluster = clust.TF.methyl$cluster %>%
  factor(levels = c(2, 4, 5, 10, 8, 11, 7, 1, 6, 3, 9))
  
col = rev(brewer.pal(5, "RdPu"));
figure_size = list(title_gp = gpar(fontsize = 18), labels_gp = gpar(fontsize = 14))

h.TF.methyl = Heatmap(tmp.cor, name = 'Scaled Corr', column_title = 'Transcription Factors-Methylation probes', 
        cluster_columns = F, cluster_rows=T, split = clust.TF.methyl$cluster, 
        col = colorRamp2(c(-1,-0.75,-0.5,-0.25,0), col),
        column_title_gp = gpar(fontsize = 22, fontface = "bold"), 
        row_title_gp = gpar(fontsize = 22, fontface = "bold"),
        column_names_gp = gpar(fontsize = 18), combined_name_fun = NULL,
        ...
        show_row_names = F, row_names_side = 'right', show_column_names = T, show_row_hclust = F,
        heatmap_legend_param = figure_size)
draw(h.TF.methyl, heatmap_legend_side = "left")
WGCNA::collectGarbage()
```

### Calculate differential co-expression analysis between miRNA-methylation features at each diffstate
Performing analysis with `r dim(mirna.methyl.coexp)[1]` mirna-methylation features calculated from `r dim(mirna.methyl.metadata)[1]` samples
```{r coexpp.fisher.mirna.methyl, fig.height=15, fig.width=15, echo=FALSE, results='asis'}
# Get all possible diff state comparisons
all.comparisons = t(combn(unique(mirna.methyl.metadata$Diffname_short), 2))

# Perform Fisher analysis across all possible diff state comparisons
mirna.methyl.fisher = apply(all.comparisons, 1, changeCoExp, 
                        mirna.methyl.metadata, mirna.methyl.coexp, 
                        var1 = 'mirna', var2 = 'methyl', FDR = 0.05, logFC = 3)
names(mirna.methyl.fisher) = apply(all.comparisons, 1, paste, collapse = '_vs_')

writeLines("Number of differentially co-expressed features at an FDR <= 0.05 and absolute logFC >= 3 are")
tmp = sapply(mirna.methyl.fisher, function(x){dim(x)[1]}, simplify = F) %>% 
  ldply(function(x){data.frame(value = x)}, .id = 'Comparison')
kable(tmp)

# Extract co-expression matrix at each differentiation stage
uniqueFeatures = lapply(mirna.methyl.fisher, function(x){
  dplyr::select(x, feature) %>%
    unlist
}) 
writeLines(paste("Total number of differentially co-expressed features at an FDR <= 0.05 and absolute logFC >= 3 are",
...
```{r cluster.mirna.methyl, fig.height=15, fig.width=15, results='asis', echo=FALSE}
tmp = mirna.methyl.coexp %>%
  dplyr::filter(feature %in% (uniqueFeatures %>% unlist %>% unique)) %>%
  dplyr::select(feature, SC, DE, MESO5, ECTO, EB)
...
writeLines(paste('Clustering',dim(tmp.cor)[1],'mirna-methylation interactions'))
# clust.mirna.methyl = pamk(tmp.cor, 1:20, seed = 12345, usepam= T)
set.seed(12345); clust.mirna.methyl = kmeans(tmp.cor, 10)
clust.mirna.methyl$cluster = clust.mirna.methyl$cluster %>%
  factor(levels = c(10, 4, 8, 7, 9, 5, 2, 3, 6, 1))
  
col = rev(brewer.pal(5, "RdPu"));
figure_size = list(title_gp = gpar(fontsize = 18), labels_gp = gpar(fontsize = 14))

h.mirna.methyl = Heatmap(tmp.cor, name = 'Scaled Corr', column_title = 'miRNA-Methylation probes', 
        cluster_columns = F, cluster_rows=T, split = clust.mirna.methyl$cluster, 
        col = colorRamp2(c(-1,-0.75,-0.5,-0.25,0), col),
        column_title_gp = gpar(fontsize = 22, fontface = "bold"), 
        row_title_gp = gpar(fontsize = 22, fontface = "bold"),
        column_names_gp = gpar(fontsize = 18), combined_name_fun = NULL,
        ...
        show_row_names = F, row_names_side = 'right', show_column_names = T, show_row_hclust = F,
        heatmap_legend_param = figure_size)
draw(h.mirna.methyl, heatmap_legend_side = "left")
WGCNA::collectGarbage()
```
...
# Store coexpression
changeCoExp <- list(TF_mirna = TF.mirna.fisher,
