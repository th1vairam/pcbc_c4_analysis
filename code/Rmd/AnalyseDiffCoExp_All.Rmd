...
clust.TF.miRNA$cluster = clust.TF.miRNA$cluster %>%
  revalue(c('6' = 'SC', '7' = 'DE', '4' = 'MESO5', '1' = 'ECTO', '3' = 'MESO15', '2' = 'MESO30', '5' = 'EB'))
```
...
```{r cluster.TF.methyl, fig.height=5, fig.width=15, results='asis', echo=FALSE}
tmp = D %>%
  dplyr::filter(feature %in% (uniqueFeatures %>% unlist %>% unique)) %>%
...
set.seed(12345); clust.TF.methyl = kmeans(tmp[,-(1)], 1)
...
clust.TF.methyl$cluster = c('FOXL1_cg02141498' = 'DE', 'FOXL1_cg02141498' = 'EB', 
                            'TBL1XR1_cg10747185' = 'MESO5', 'TBL1XR1_cg10747185' = 'ECTO') 
WGCNA::collectGarbage()
```

### Calculate differential co-expression analysis between nonTF-miRNA features at each diffstate
Performing analysis with `r dim(nonTF.mirna.coexp)[1]` nonTF-miRNA interactions calculated from `r dim(mrna.mirna.metadata)[1]` samples
```{r coexpp.fisher.nonTF.miRNA, fig.height=15, fig.width=15, echo=FALSE, results='asis'}
# Get all possible diff state comparisons
all.comparisons = t(combn(setdiff(unique(mrna.mirna.metadata$Diffname_short), c()),2))#'MESO15','MESO30'

# Perform Fisher analysis across all possible diff state comparisons
nonTF.mirna.fisher = apply(all.comparisons, 1, changeCoExp, 
                        mrna.mirna.metadata, nonTF.mirna.coexp, 
                        var1 = 'mrna', var2 = 'mirna', FDR = 0.05, logFC = 0)
names(nonTF.mirna.fisher) = apply(all.comparisons, 1, paste, collapse = '_vs_')
...
D = mapply(function(x,y){
  ...
  x = plyr::rename(x, c(correlation.group1 = group[1], correlation.group2 = group[2])) %>%
    dplyr::select(one_of(c('feature','mrna','mirna',group)))
}, nonTF.mirna.fisher, names(nonTF.mirna.fisher), SIMPLIFY = F) %>%
  plyr::join_all(type = "full", by = c("feature", "mrna", "mirna"))
...
tmp = sapply(nonTF.mirna.fisher, function(x, diffexp){
...
# Extract co-expression matrix at each differentiation stage
uniqueFeatures = lapply(nonTF.mirna.fisher, function(x, diffexp){
  mRNA = (intersect(x$mrna, diffexp[['nonTF']]$feature))
  miRNA = (intersect(x$mirna, diffexp[['mirna']]$feature))
  x = dplyr::filter(x, mrna %in% mRNA | mirna %in% miRNA) %>%
    dplyr::select(feature) %>%
...
nonTF.mirna.fisher = ldply(nonTF.mirna.fisher, .id = "Comparison")
```

```{r cluster.nonTF.miRNA, fig.height=35, fig.width=15, results='asis', echo=FALSE, cache=FALSE}
tmp = D %>%
  dplyr::filter(feature %in% (uniqueFeatures %>% unlist %>% unique)) %>%
  dplyr::select(feature, SC, DE, MESO5, ECTO, MESO15, MESO30, EB)
...
writeLines(paste('Clustering',dim(tmp.cor)[1],'nonTF-miRNA interactions'))
clust.nonTF.miRNA = pamk(tmp.cor, 1:20, seed = 12345, usepam= F)
set.seed(12345); clust.nonTF.miRNA = kmeans(tmp.cor, 6)#clust.nonTF.miRNA$nc)
clust.nonTF.miRNA$cluster = clust.nonTF.miRNA$cluster %>%
  factor(levels = c(1,2,3,5,4,6))
...
h.nonTF.miRNA = Heatmap(tmp[,-(1)], name = 'Corr', 
        cluster_columns = F, cluster_rows=T, split = clust.nonTF.miRNA$cluster, 
        # col = colorRamp2(c(-1,-0.75,-0.5,-0.25,0), col),
        column_title_gp = gpar(fontsize = 22, fontface = "bold"), 
        row_title_gp = gpar(fontsize = 22, fontface = "bold"),
        column_names_gp = gpar(fontsize = 18), combined_name_fun = NULL,
        row_names_gp = gpar(fontsize = 10),
        show_row_names = F, row_names_side = 'right', show_column_names = T, show_row_hclust = T,
        heatmap_legend_param = figure_size)
draw(h.nonTF.miRNA, heatmap_legend_side = "left", column_title = 'Non Transcription Factors-miRNA',
     column_title_gp = gpar(fontsize = 22, fontface = "bold"))
WGCNA::collectGarbage()
```

### Calculate differential co-expression analysis between nonTF-methylation interactions at each diffstate
Performing analysis with `r dim(nonTF.methyl.coexp)[1]` nonTF-methylation interactions calculated from `r dim(mrna.methyl.metadata)[1]` samples
```{r coexpp.fisher.nonTF.methyl, fig.height=15, fig.width=15, echo=FALSE, results='asis'}
# Get all possible diff state comparisons
all.comparisons = t(combn(setdiff(unique(mrna.methyl.metadata$Diffname_short), c()),2))#'MESO15','MESO30'

# Perform Fisher analysis across all possible diff state comparisons
nonTF.methyl.fisher = apply(all.comparisons, 1, changeCoExp, 
                            mrna.methyl.metadata, nonTF.methyl.coexp, 
                            var1 = 'mrna', var2 = 'methyl', FDR = 0.05, logFC = 0)
names(nonTF.methyl.fisher) = apply(all.comparisons, 1, paste, collapse = '_vs_')
...
D = mapply(function(x,y){
  ...
  x = plyr::rename(x, c(correlation.group1 = group[1], correlation.group2 = group[2])) %>%
    dplyr::select(one_of(c('feature','mrna','methyl',group)))
}, nonTF.methyl.fisher, names(nonTF.methyl.fisher), SIMPLIFY = F) %>%
  plyr::join_all(type = "full", by = c("feature", "mrna", "methyl"))
...
tmp = sapply(nonTF.methyl.fisher, function(x, diffexp){
  mRNA = (intersect(x$mrna, diffexp[['nonTF']]$feature))
  methyl = (intersect(x$methyl, diffexp[['methyl']]$feature))
  x = dplyr::filter(x, mrna %in% mRNA | methyl %in% methyl)
  dim(x)[1]
...
# Extract co-expression matrix at each differentiation stage
uniqueFeatures = lapply(nonTF.methyl.fisher, function(x, diffexp){
  mRNA = (intersect(x$mrna, diffexp[['nonTF']]$feature))
  methyl = (intersect(x$methyl, diffexp[['methyl']]$feature))
  x = dplyr::filter(x, mrna %in% mRNA | methyl %in% methyl) %>%
    dplyr::select(feature) %>%
...
nonTF.methyl.fisher = ldply(nonTF.methyl.fisher, .id = "Comparison")
```

```{r cluster.nonTF.methyl, fig.height=25, fig.width=15, results='asis', echo=FALSE}
tmp = D %>%
  dplyr::filter(feature %in% (uniqueFeatures %>% unlist %>% unique)) %>%
  dplyr::select(feature, SC, DE, MESO5, ECTO, EB)
...
writeLines(paste('Clustering',dim(tmp.cor)[1],'nonTF-miRNA interactions'))
clust.nonTF.methyl = pamk(tmp.cor, 1:20, seed = 12345, usepam= F)
set.seed(12345); clust.nonTF.methyl = kmeans(tmp.cor, 6)#clust.nonTF.methyl$nc)
clust.nonTF.methyl$cluster = clust.nonTF.methyl$cluster %>%
...
h.nonTF.methyl = Heatmap(tmp[,-(1)], name = 'Corr', 
        cluster_columns = F, cluster_rows=T, split = clust.nonTF.methyl$cluster, 
        # col = colorRamp2(c(-1,-0.75,-0.5,-0.25,0), col),
        column_title_gp = gpar(fontsize = 22, fontface = "bold"), 
        row_title_gp = gpar(fontsize = 22, fontface = "bold"),
        column_names_gp = gpar(fontsize = 18), combined_name_fun = NULL,
        ...
        show_row_names = T, row_names_side = 'right', show_column_names = T, show_row_hclust = F,
        heatmap_legend_param = figure_size)
draw(h.nonTF.methyl, heatmap_legend_side = "left", column_title = 'Non Transcription Factors-Methylation Probes',
     column_title_gp = gpar(fontsize = 22, fontface = "bold"))
```
...
changeCoexp <- list(TF_mirna = TF.mirna.fisher,
                    ...
                    nonTF_mirna = nonTF.mirna.fisher,
                    nonTF_methyl = nonTF.methyl.fisher,
