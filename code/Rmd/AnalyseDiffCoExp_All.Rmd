...
Dicordant features at P.value <= 0.05
...
names(results.discordant) = apply(all.comparisons, 1, paste, collapse = '_vs_')

diffFeatures = lapply(results.discordant, function(x){
  tmp = x$discordPPMatrix$feature[x$discordPPMatrix$Pvalue <= 0.05] %>% 
    paste(collapse = ',')
  tmp = gsub('_',':',tmp)
}) %>% ldply(function(x){data.frame(value = x)})
kable(diffFeatures)
```
### Calculate co-expression between TF-miRNA features at each diffstate
```{r coexpp, fig.height=25, fig.width=15}
