---
title: Network fusion analysis on EB and SC samples based on germline signatures
...
knit2synapse::knitToFolderEntity(file = 'NetFuse_EBandSC_GermlineSignatures.Rmd',
                                 parentId = "syn5194922",
                                 entityName = "Network Fusion Analysis on EBs and SCs with Germline Signatures")
```
...
ActivityName <- 'Decompose EBs and SCs based on germline signatures'

# Get the latest commit of this code from github
thisFileName <- 'NetFuse_EBandSC_GermlineSignatures.Rmd'
...
CODE <- Folder(name = "Network Fusion Analysis on EBs and SCs with Germline Signatures", parentId = parentId)
CODE <- synStore(CODE)
```
...
germline.sig = downloadFile('syn6828794') %>%
  filter(feature.assay %in% c('mrna','TF','methyl','mirna')) %>%
