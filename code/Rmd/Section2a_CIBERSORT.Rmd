...
Since, EBs are derivatives of pluripotent and/or germlayer states, we characterised our EBs as a mixture model of the above identified signatures but with only genes expressed atleast in 10% of EB samples
...
# Find signature genes that are atleast expressed in 10% of EB samples
ind.10percent = which(rowSums(tmp[,covariates$mrna$UID[covariates$mrna$Diffname_short == 'EB']] <= 0) > 5)
ind.signatures = which(rownames(tmp) %in% state.specific.signatures$feature)
ind = intersect(ind.10percent, ind.signatures)

# Subset mrna expression
tmp.EB = tmp[ind, covariates$mrna$UID[covariates$mrna$Diffname_short == 'EB']]
tmp.notEB = tmp[ind, covariates$mrna$UID[covariates$mrna$Diffname_short %in% c('DE','MESO15','MESO30','ECTO')]]
...
writeLines('Relationship between meso scores from cibersort and pc1 from the decomposition of signatures')
p = ggplot(tmp, aes(y = PC1, x = MESO)) + geom_point() + geom_smooth(method = lm)
p = p + geom_text(aes(label=C4_Cell_Line_ID),hjust=0, vjust=0)
p
```
...
covars = covariates$mrna %>%
  ...
  dplyr::select(UID, C4_Cell_Line_ID, Cell_Line_Type, Cell_Line_of_Origin, Tissue_of_Origin, 
...
plot(covars1$Cell_Line_Type, covars1$DE, ylab = 'Fraction of DE')
```
Association between cell line type is observed