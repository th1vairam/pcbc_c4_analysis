...
```{r cibersort.mixed, include=FALSE}
# Standard normalisation of signatures
...
```{r cibersort.output, fig.height=10, fig.width=10}
cibersort.results = downloadFile('syn7187931') %>%
  plyr::rename(c('Input Sample' = 'Input_Sample')) %>%
  dplyr::select(Input_Sample, SC, DE, MESO, ECTO) %>%
  gather(DiffState, Value, -Input_Sample) %>%
  dplyr::rename(Fraction = Value, DifferentiationStage = DiffState, C4_Cell_Line_ID = Input_Sample) 

p = ggplot(cibersort.results, aes(x = C4_Cell_Line_ID, y = Fraction, fill = DifferentiationStage)) + geom_bar(stat = 'identity')
p = p + theme(axis.text.x = element_text(angle = 90, hjust = 1))
p
```

Association test between different covariates and stratified diff state scores of EBs
```{r associate.with.covars}
covars = covariates$mrna %>%
  ...
  right_join( cibersort.results %>%
                separate(C4_Cell_Line_ID, c('C4_Cell_Line_ID','Diffname_short'), sep = '\\.') %>%
                spread(DifferentiationStage, Fraction) ) %>%
  as.data.frame()
