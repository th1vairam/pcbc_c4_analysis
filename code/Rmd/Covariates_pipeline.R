#### Covariates pipelines for PCBC RNA-Seq expresison analysis
# Clear Screen
cat("\014")  
...
# Extra environment specific options
...
FactorCovariates = c('Diffname_short','run','lane','index','Cell_Line_Type','Cell_Type_of_Origin','Tissue_of_Origin',
                     'Reprogramming_Vector_Type','Reprogramming_Gene_Combination','Donor_Life_Stage','C4_Karyotype_Result',
...
##############################################################################################################################
#### Normalisation: Determine desing matrix for normalisation and differential expression analysis ####

# Remove genes that have less than 1 cpm counts in atleast 50% of samples
PROCESSED_COUNTS = getGeneFilteredGeneExprMatrix(COUNT)
pdf('CPM_HISTOGRAM.pdf')
...
pdf('COVARIATES_CLUSTERING.pdf',width=12,height=10)
...
pdf('INITIAL_PCA.pdf',width=20,height=10)
eval(PC_res)
dev.off()

# Re-normalise with COVARIATES desing matrix
DM = getDesignMatrix(METADATA[,c('Diffname_short','run','lane','index','Cell_Line_Type','Cell_Type_of_Origin','Donor_Life_Stage')],FactorCovariates)
...
pdf('ADJUSTED1_PCA.pdf',width=20,height=10)
eval(PC_res)
dev.off()
...
PC_res=list()
postAdjustedSigCovars = runPCAandPlotCorrelations(t(result.lm$residuals), DM$design, 'residual',isKeyPlot=TRUE)
pdf('RESIDUAL_PCA.pdf',width=20,height=10)
eval(PC_res)
dev.off()