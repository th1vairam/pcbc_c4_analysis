...
metadata = syn.query('select * from entity where diffnameshort=="SC" and inductiongenes=="OSKM - NLT" and parentId=="%s"' %METADATA_ID)['results']
...
display.HTML(metadata[['bamName', 'celltypeoforigin', 'celltissuesourcepreinduction', 'inductiongenes', 'diffnameshort', 'lane', 'index', 'donorsex.cell.lines', 'numberofreads', 'uniquelyalignedreads', 'ratio']].to_html())
...
u, s, vt = QaD_SVD(dataNorm, colorLabels = metaColor, labels=metaColor)
print '\n'.join(['%s \t%0.4f' %(name, val) for name, val in zip(metadata['bamName'], vt[0,:])])
...
for i in range(3):
    ...
    topGenes['Corr PC %i' %(i+1)] = geneNames[topGeneIdx][:100]
    topGenes['AntiCorr PC %i' %(i+1)] = geneNames[topGeneIdx][-100:]
del topGenes['none']
